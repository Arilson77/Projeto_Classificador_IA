...
# ============================================================================
# 🎨 ABA MODELO ML - INTERFACE SIMPLIFICADA
# ============================================================================

aba_modelo_ml <- tabItem(
  tabName = "modelo_ml",
  
  fluidRow(
    column(12,
      box(
        title = div(icon("brain"), " Sistema de Modelo Machine Learning"),
        status = "primary",
        solidHeader = TRUE,
        width = 12,
        ...
        div(
          ...
          h4("📝 Validação de Classificações"),
          ...
          h4("🧪 Teste Personalizado"),
          
          textAreaInput(
            "texto_teste_ml",
            "Digite um texto para testar:",
            rows = 3,
            placeholder = "Ex: Substituição de válvula por falha..."
          ),
...
# ============================================================================
# 🎨 NOVA ABA: MODELO ML - INTEGRADA COM SEU DASHBOARD EXISTENTE
# ============================================================================

# Esta aba será adicionada ao seu tabItems existente
modelo_ml_tab <- tabItem(
  tabName = "modelo_ml",
...
# Adicionado automaticamente pelo script de integração
# Data: 10/12/2025 15:17:59
# ============================================================================

# Verificar se já foi integrado (evitar duplicação)
if(!exists("MODELO_TREINADO_INTEGRADO")) {
  
  cat("\n🤖 CARREGANDO SISTEMA DE MODELO TREINADO...\n")
  cat("═══════════════════════════════════════════════════════════════\n")
  
  # Bibliotecas adicionais para ML (instalar se necessário)
  if(!require(randomForest, quietly = TRUE)) {
    cat("📦 Instalando randomForest...\n")
    install.packages("randomForest", dependencies = TRUE)
    library(randomForest)


# ============================================================================
# 🤖 SISTEMA DE MODELO MACHINE LEARNING - VERSAO CORRIGIDA
# Integrado ao CLASSIFICADOR_VERSAO12 - Arilson Rodrigues Alves
# Data: 10/12/2025 15:28:46
# ============================================================================

if(!exists("MODELO_ML_CARREGADO")) {
  
  cat("\n🤖 CARREGANDO SISTEMA DE MODELO ML...\n")
  cat("═══════════════════════════════════════════════════════════════\n")
  ...
  cat("✅ Bibliotecas ML carregadas\n")
  
  # Estrutura reativa para o modelo
  ...
  cat("✅ Estrutura reativa criada\n")
  
  # Função para salvar validação
  salvar_validacao_ml <- function(id_registro, tipo_correto) {
    ...
    tryCatch({
      ...
      registro <- values$resultados_lote[values$resultados_lote$nota_key == id_registro, ]
      
      if(nrow(registro) == 0) {
        return(FALSE)
      }
      
      registro <- registro[1, ]  # Primeiro registro se houver duplicatas
      
      nova_validacao <- data.frame(
        id = id_registro,
        texto = as.character(registro$texto_completo),
        tipo_original = ifelse(is.na(registro$tipo_intervencao_antigo), 0, as.integer(registro$tipo_intervencao_antigo)),
        tipo_ia = as.integer(registro$tipo_novo),
        tipo_correto = as.integer(tipo_correto),
        confianca = ifelse(is.na(registro$confianca), 0, as.numeric(registro$confianca)),
        timestamp = Sys.time(),
        stringsAsFactors = FALSE
      )
      
      # Verificar se já existe
      if(id_registro %in% modelo_ml_dados$validacoes$id) {
        idx <- which(modelo_ml_dados$validacoes$id == id_registro)
        modelo_ml_dados$validacoes[idx, ] <- nova_validacao
      } else {
        modelo_ml_dados$validacoes <- rbind(modelo_ml_dados$validacoes, nova_validacao)
      }
      
      # Salvar em arquivo
      salvar_arquivo_ml()
      
      cat("✅ Validação salva:", id_registro, "→", tipo_correto, "\n")
      return(TRUE)
  ...
  salvar_arquivo_ml <- function() {
    tryCatch({
      
      if(!dir.exists("dados_ml")) {
        dir.create("dados_ml")
      }
      
      dados_salvar <- list(
        validacoes = modelo_ml_dados$validacoes,
        modelo = modelo_ml_dados$modelo,
        metricas = modelo_ml_dados$metricas,
        timestamp = Sys.time()
      )
      
      arquivo <- file.path("dados_ml", paste0("modelo_", format(Sys.Date(), "%Y%m%d"), ".rds"))
      saveRDS(dados_salvar, arquivo)
  ...
  carregar_arquivo_ml <- function() {
    tryCatch({
      ...
      arquivos <- list.files("dados_ml", pattern = "modelo_.*\\.rds$", full.names = TRUE)
      ...
      modelo_ml_dados$validacoes <- dados$validacoes
      modelo_ml_dados$modelo <- dados$modelo
      modelo_ml_dados$metricas <- dados$metricas
      
      cat("✅ Dados carregados:", nrow(modelo_ml_dados$validacoes), "validações\n")
      return(TRUE)
  ...
  treinar_modelo_ml <- function() {
    ...
    if(total < modelo_ml_dados$configuracao$min_validacoes) {
      return(list(
        sucesso = FALSE,
        erro = paste("Necessário pelo menos", modelo_ml_dados$configuracao$min_validacoes, "validações")
      ))
    }
    
    tryCatch({
      
      cat("🚀 Treinando modelo ML...\n")
      
      # Preparar dados
      dados <- modelo_ml_dados$validacoes
      dados <- dados[!is.na(dados$tipo_correto) & nchar(dados$texto) > 5, ]
      ...
      dados$tamanho <- nchar(dados$texto)
      dados$palavras <- lengths(strsplit(dados$texto, "\\s+"))
      dados$tem_limpeza <- as.numeric(grepl("limp|lavar", tolower(dados$texto)))
      dados$tem_substituicao <- as.numeric(grepl("substitu|troca", tolower(dados$texto)))
      dados$tem_inspecao <- as.numeric(grepl("inspe|verif", tolower(dados$texto)))
      dados$tem_falha <- as.numeric(grepl("falha|problem", tolower(dados$texto)))
      dados$tipo_correto <- as.factor(dados$tipo_correto)
      
      # Treinar Random Forest
      modelo_rf <- randomForest(
        tipo_correto ~ tamanho + palavras + tem_limpeza + tem_substituicao + tem_inspecao + tem_falha + confianca,
        data = dados,
        ntree = 50,
        importance = TRUE
      )
      ...
      # Calcular acurácia
      predicoes <- predict(modelo_rf, dados)
      acuracia <- round(mean(predicoes == dados$tipo_correto) * 100, 2)
      
      # Salvar modelo
      modelo_ml_dados$modelo <- modelo_rf
      modelo_ml_dados$metricas$acuracia <- acuracia
      modelo_ml_dados$metricas$total_dados <- nrow(dados)
      modelo_ml_dados$metricas$ultima_atualizacao <- Sys.time()
      
      # Salvar arquivo
      salvar_arquivo_ml()
      
      cat("✅ Modelo treinado! Acurácia:", acuracia, "%\n")
      
      return(list(
        sucesso = TRUE,
        acuracia = acuracia,
        total_dados = nrow(dados)
      ))
  ...
  predizer_ml <- function(texto) {
    ...
    tryCatch({
      ...
      features <- data.frame(
        tamanho = nchar(texto),
        palavras = length(strsplit(texto, "\\s+")[[1]]),
        tem_limpeza = as.numeric(grepl("limp|lavar", tolower(texto))),
        tem_substituicao = as.numeric(grepl("substitu|troca", tolower(texto))),
        tem_inspecao = as.numeric(grepl("inspe|verif", tolower(texto))),
        tem_falha = as.numeric(grepl("falha|problem", tolower(texto))),
        confianca = 80
      )
      
      # Predição
      predicao <- predict(modelo_ml_dados$modelo, features)
      probs <- predict(modelo_ml_dados$modelo, features, type = "prob")
      
      confianca <- round(max(probs) * 100, 2)
      
      return(list(
        sucesso = TRUE,
        tipo = as.numeric(as.character(predicao)),
        confianca = confianca
      ))
  ...
  cat("🎉 SISTEMA ML CARREGADO COM SUCESSO!\n")
  cat("═══════════════════════════════════════════════════════════════\n\n")
}
  ...
  # Estrutura reativa para validações e modelo
  ...
  # Adicionar configurações ao CONFIG_USUARIO existente
  if(exists("CONFIG_USUARIO")) {
    CONFIG_USUARIO$modelo_treinado <- list(
      ativo = FALSE,
      usar_automaticamente = FALSE,
      confianca_minima = 70,
      prioridade_sobre_dicionario = FALSE,
      prioridade_sobre_api = FALSE
    )
    cat("✅ Configurações adicionadas ao CONFIG_USUARIO\n")
  }
  
  # Função para salvar validação (integrada com seu sistema existente)
  salvar_validacao_ml <- function(registro_id, tipo_validado, assunto_validado = NULL, 
                                 feedback = "boa", observacoes = "")  {
    
    tryCatch({
      
      # Verificar se há resultados do lote atual
      if(is.null(values$resultados_lote) || nrow(values$resultados_lote) == 0) {
        cat("⚠️ Nenhum resultado de lote disponível\n")
        return(FALSE)
      }
      
      # Buscar registro original
      registro <- values$resultados_lote %>%
        filter(nota_key == registro_id) %>%
        slice(1)
      
      if(nrow(registro) == 0) {
        cat("⚠️ Registro", registro_id, "não encontrado\n")
        return(FALSE)
      }
      ...
      nova_validacao <- data.frame(
        id = registro_id,
        texto_original = registro$texto_completo,
        tipo_original = ifelse(is.na(registro$tipo_intervencao_antigo), 0, as.integer(registro$tipo_intervencao_antigo)),
        tipo_ia = as.integer(registro$tipo_novo),
        tipo_validado = as.integer(tipo_validado),
        assunto_original = ifelse(is.na(registro$assunto_principal), "", as.character(registro$assunto_principal)),
        assunto_validado = ifelse(is.null(assunto_validado) || nchar(trimws(assunto_validado)) == 0,
                                 ifelse(is.na(registro$assunto_principal), "", as.character(registro$assunto_principal)),
                                 assunto_validado),
        confianca = ifelse(is.na(registro$confianca), 0, registro$confianca),
      ...
      # Verificar se já existe
      if(registro_id %in% validacoes_modelo$dados$id) {
        idx <- which(validacoes_modelo$dados$id == registro_id)
        validacoes_modelo$dados[idx, ] <- nova_validacao
        cat("🔄 Validação atualizada:", registro_id, "\n")
      } else {
        validacoes_modelo$dados <- rbind(validacoes_modelo$dados, nova_validacao)
        cat("✅ Nova validação:", registro_id, "→ Tipo", tipo_validado, "\n")
      }
  ...
  carregar_dados_modelo <- function()  {
    tryCatch({
      ...
      if(!is.null(dados$configuracoes)) {
        validacoes_modelo$configuracoes <- dados$configuracoes
      }
      
      cat("✅ Dados carregados:", nrow(validacoes_modelo$dados), "validações\n")
      
      return(TRUE)
      
    }, error = function(e)  {
      cat("⚠️ Erro ao carregar dados:", as.character(e), "\n")
      return(FALSE)
    })
  }
  
  # Função para treinar modelo ML
  treinar_modelo_ml <- function(force_retrain = FALSE)  {
    ...
    if(total_validacoes < validacoes_modelo$configuracoes$min_validacoes) {
      return(list(
        sucesso = FALSE,
        erro = paste("Necessário pelo menos", validacoes_modelo$configuracoes$min_validacoes, 
    ...
    tryCatch({
      ...
      # Preparar dados
      dados_limpos <- validacoes_modelo$dados %>%
        filter(!is.na(tipo_validado), !is.na(texto_original), 
               nchar(trimws(texto_original)) > 10) %>%
        mutate(tipo_validado = as.factor(tipo_validado))
      
      cat("📊 Dados preparados:", nrow(dados_limpos), "registros\n")
      
      if(nrow(dados_limpos) < 5) {
        return(list(
          sucesso = FALSE,
          erro = "Dados insuficientes após limpeza (mínimo 5)"
        ))
      }
      
      # Criar features de texto simples
      cat("🔧 Extraindo features...\n")
      
      dados_features <- dados_limpos %>%
        mutate(
          # Features básicas de texto
          tamanho_texto = nchar(texto_original),
          num_palavras = str_count(texto_original, "\\S+"),
          tem_limpeza = as.numeric(grepl("limp|lavar|higien", tolower(texto_original))),
          tem_substituicao = as.numeric(grepl("substitu|troca|repair", tolower(texto_original))),
          tem_inspecao = as.numeric(grepl("inspe|verif|test", tolower(texto_original))),
          tem_modificacao = as.numeric(grepl("modif|alter|ajust", tolower(texto_original))),
          tem_falha = as.numeric(grepl("falha|problem|defeito", tolower(texto_original))),
          tem_emergencia = as.numeric(grepl("emerg|urgent|critic|parada", tolower(texto_original))),
          confianca_ia = confianca
        )
      
      # Dividir em treino e teste
      set.seed(123)
      indices_treino <- createDataPartition(dados_features$tipo_validado, p = 0.8, list = FALSE)
      
      treino <- dados_features[indices_treino, ]
      teste <- dados_features[-indices_treino, ]
      
      cat("📈 Divisão - Treino:", nrow(treino), "| Teste:", nrow(teste), "\n")
      
      # Treinar Random Forest
      cat("🌲 Treinando Random Forest...\n")
      
      formula_modelo <- tipo_validado ~ tamanho_texto + num_palavras + tem_limpeza + 
                       tem_substituicao + tem_inspecao + tem_modificacao + 
                       tem_falha + tem_emergencia + confianca_ia
      ...
      if(nrow(teste) > 0) {
        ...
        matriz <- confusionMatrix(predicoes, teste$tipo_validado)
        acuracia <- round(matriz$overall['Accuracy'] * 100, 2)
      } else {
        # Se não há teste, usar validação cruzada simples
        acuracia <- round(mean(modelo_rf$err.rate[, 1]) * 100, 2)
        acuracia <- max(0, 100 - acuracia)  # Converter erro para acurácia
      }
      
      cat("📊 Acurácia obtida:", acuracia, "%\n")
      
      # Salvar modelo
      validacoes_modelo$modelo_ativo <- modelo_rf
      validacoes_modelo$metricas$acuracia <- acuracia
      validacoes_modelo$metricas$total_treinos <- nrow(dados_limpos)
      validacoes_modelo$metricas$ultima_atualizacao <- Sys.time()
      
      # Features importantes
      importancia <- importance(modelo_rf)
      validacoes_modelo$metricas$features_importantes <- rownames(importancia)[order(importancia[,1], decreasing = TRUE)]
      
      # Adicionar ao histórico
      novo_treino <- list(
        timestamp = Sys.time(),
        acuracia = acuracia,
        total_dados = nrow(dados_limpos),
        algoritmo = "randomForest"
      )
      validacoes_modelo$historico <- append(validacoes_modelo$historico, list(novo_treino))
      
      # Salvar tudo
      salvar_dados_modelo()
      
      cat("💾 Modelo salvo com sucesso!\n")
      cat("══════════════════════════════════════\n\n")
  ...
  # Função para predizer com modelo treinado
  predizer_com_modelo <- function(texto)  {
    
    if(is.null(validacoes_modelo$modelo_ativo)) {
      return(list(
        sucesso = FALSE,
        erro = "Nenhum modelo treinado disponível"
      ))
    }
    
    tryCatch({
      
      # Criar features para o texto
      features_texto <- data.frame(
        tamanho_texto = nchar(texto),
        num_palavras = str_count(texto, "\\S+"),
        tem_limpeza = as.numeric(grepl("limp|lavar|higien", tolower(texto))),
        tem_substituicao = as.numeric(grepl("substitu|troca|repair", tolower(texto))),
        tem_inspecao = as.numeric(grepl("inspe|verif|test", tolower(texto))),
        tem_modificacao = as.numeric(grepl("modif|alter|ajust", tolower(texto))),
        tem_falha = as.numeric(grepl("falha|problem|defeito", tolower(texto))),
        tem_emergencia = as.numeric(grepl("emerg|urgent|critic|parada", tolower(texto))),
        confianca_ia = 80  # Valor padrão
      )
      
      # Predição
      predicao <- predict(validacoes_modelo$modelo_ativo, features_texto)
      prob <- predict(validacoes_modelo$modelo_ativo, features_texto, type = "prob")
  ...
  # Expandir função classificar_hibrido existente
  if(exists("classificar_hibrido")) {
    
    # Salvar função original
    classificar_hibrido_original <- classificar_hibrido
    
    # Nova função que integra modelo treinado
    classificar_hibrido <- function(texto, config = CONFIG_USUARIO)  {
      
      # Resultado do método original (seu sistema atual)
      resultado_original <- classificar_hibrido_original(texto, config)
      
      # Verificar se deve usar modelo treinado
      usar_modelo <- FALSE
      
      if(exists("CONFIG_USUARIO") && !is.null(CONFIG_USUARIO$modelo_treinado)) {
        usar_modelo <- isTRUE(CONFIG_USUARIO$modelo_treinado$usar_automaticamente)
      }
      
      # Se modelo não está ativo, retornar resultado original
      if(!usar_modelo || is.null(validacoes_modelo$modelo_ativo)) {
        return(resultado_original)
      }
      
      # Tentar predição com modelo
      predicao_modelo <- predizer_com_modelo(texto)
      
      if(!predicao_modelo$sucesso) {
        # Erro no modelo, usar resultado original
        resultado_final <- resultado_original
        resultado_final$metodo <- paste0(resultado_original$metodo, "_MODELO_ERRO")
        return(resultado_final)
      }
      
      # Comparar resultados
      concorda <- (predicao_modelo$tipo == resultado_original$tipo)
      
      if(concorda) {
        
        # Modelo e método original concordam - aumentar confiança
        confianca_combinada <- (resultado_original$confianca * 0.6 + predicao_modelo$confianca * 0.4)
        
        resultado_final <- resultado_original
        resultado_final$confianca <- min(99, confianca_combinada)
        resultado_final$metodo <- paste0(resultado_original$metodo, "+MODELO_CONCORDANTE")
        ...
        if(predicao_modelo$confianca > resultado_original$confianca) {
          
          # Modelo tem maior confiança
          resultado_modelo <- list(
            tipo = predicao_modelo$tipo,
            categoria = ifelse(predicao_modelo$tipo %in% c(5, 6), "IAZF", "PROBLEMAS_COMUNS"),
            criticidade = switch(
              as.character(predicao_modelo$tipo),
              "1" = "BAIXA", "2" = "BAIXA", "3" = "MEDIA",
              "4" = "MEDIA", "5" = "ALTA", "6" = "CRITICA"
            ),
            confianca = predicao_modelo$confianca,
            descricao = ifelse(exists("CONFIG_USUARIO") && 
                             !is.null(CONFIG_USUARIO$dicionarios[[paste0("tipo_", predicao_modelo$tipo)]]),
                             CONFIG_USUARIO$dicionarios[[paste0("tipo_", predicao_modelo$tipo)]]$descricao,
                             paste("Tipo", predicao_modelo$tipo, "identificado pelo modelo treinado")),
            resumo = paste("Classificado pelo modelo treinado com", predicao_modelo$confianca, "% de confiança"),
            metodo = "MODELO_TREINADO_PRIORITARIO"
          )
          ...
          # Método original tem maior confiança
          resultado_final <- resultado_original
          resultado_final$metodo <- paste0(resultado_original$metodo, "+MODELO_DIVERGENTE")
    ...
    cat("✅ Função classificar_hibrido expandida com modelo ML\n")
  }
  ...
  cat("🎉 SISTEMA DE MODELO TREINADO INTEGRADO COM SUCESSO!\n")
  cat("💡 Nova aba 'Modelo ML' será adicionada à interface\n")
  cat("═══════════════════════════════════════════════════════════════\n\n")
}
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            solidHeader = TRUE,
            width = 12,
            style = "margin-top: 20px; margin-bottom: 20px;",  # ← Margem superior E inferior
            div(
              style = "padding: 25px;",
        ...
        #===========================================================================
        # ABA: PROCESSAMENTO EM LOTE (CORRIGIDA)
        #===========================================================================
        ...
        tabItem(
          ...
          # ✅ SEÇÃO DE CONFIGURAÇÃO (SEMPRE VISÍVEL)
          fluidRow(
            box(
              title = div(
              ...
              fluidRow(
                column(
                  ...
                  selectInput(
                    ...
                    label = div(
                      icon("robot", style = "margin-right: 8px; color: #667eea;"),
                      strong("Método de Classificação:")
                    ),
                    choices = c(
                ...
                column(
                  ...
                  div(
                    ...
                    checkboxInput(
                      "extrair_assunto",
                      label = div(
                        icon("file-alt", style = "margin-right: 8px; color: #2196F3;"),
                        strong("Extrair Assunto Principal com IA")
                      ),
                      value = TRUE
                    )
                  )
              ...
              div(
                ...
                actionButton(
                  "classificar_lote",
                  label = div(
                    icon("rocket", style = "margin-right: 10px; font-size: 20px;"),
                    "Classificar em Lote"
                  ),
                  class = "btn-lg",
                  style = "background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                   color: white; font-weight: bold; padding: 15px 40px; 
                ...
                actionButton(
                  ...
                  class = "btn-lg",
                  style = "background: #95a5a6; color: white; font-weight: bold; 
        ...
        #===========================================================================
        # ABA 6: ESTATÍSTICAS PREMIUM COM INSIGHTS DE IA
        #===========================================================================
        ...
        tabItem(
          ...
          #===========================================================================
          # PAINEL DE INSIGHTS COM IA (NOVO!)
          #===========================================================================
          ...
          fluidRow(
            box(
              ...
              div(
                ...
                div(
                  ...
                  div(
                    ...
                    div(
                      ...
                      div(
                        style = "display: flex; align-items: center; margin-bottom: 15px;",
                        icon("magic", style = "font-size: 40px; color: #2196F3; margin-right: 20px;"),
                        div(
                          h4(style = "margin: 0 0 8px 0; color: #1565C0; font-weight: 700;",
                             "Análise Inteligente Automatizada"),
                          p(style = "margin: 0; color: #666; font-size: 14px; line-height: 1.6;",
                            "Nossa IA analisa seus dados em tempo real e gera insights profundos como um verdadeiro cientista de dados. ",
                            "Identifica padrões, anomalias e fornece recomendações acionáveis baseadas nas métricas atuais.")
                        )
                      )
                    ),
                    div(
                      ...
                      actionButton(
                        "gerar_insights",
                        label = div(
                          icon("brain", style = "margin-right: 10px; font-size: 20px;"),
                ...
                # Área de insights (será preenchida dinamicamente)
      ...
      # ============================================================================
      # NOVA ABA: VALIDAÇÃO E TREINAMENTO
      # ============================================================================
          ...
          column(
            ...
            div(
              ...
              div(
                ...
                div(
                  h1(style = "color: white; margin: 0; font-weight: 700; font-size: 32px;",
                     icon("graduation-cap", style = "margin-right: 15px;"), 
                     "Validação e Treinamento do Modelo"),
                  p(style = "color: rgba(255,255,255,0.95); margin: 10px 0 0 0; font-size: 16px;",
                    "Valide classificações e treine o modelo para maior precisão")
                ),
                div(
                  style = "text-align: right;",
                  h2(style = "color: white; margin: 0; font-weight: 800; font-size: 42px;",
                     textOutput("total_validacoes_inline", inline = TRUE)),
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              div(
                style = "padding: 25px;",
                uiOutput("status_modelo_treinado")
              )
            )
          ),
          ...
          column(
            ...
            box(
              ...
              div(
                style = "padding: 25px;",
                plotOutput("grafico_evolucao_precisao", height = "300px")
              )
            )
          )
        ),
            ...
            div(
              ...
              fluidRow(
                column(
                  ...
                  selectInput(
                    "filtro_validacao",
                    label = div(icon("filter"), " Filtrar por:"),
                    choices = c(
                      "Todos" = "todos",
                      "Não Validados" = "nao_validados", 
                      "Divergentes" = "divergentes",
                      "Baixa Confiança (<80%)" = "baixa_confianca",
                      "Críticos (IAZF)" = "criticos"
                    ),
                    selected = "nao_validados"
                  )
                ),
                column(
                  ...
                  numericInput(
                    "limite_validacao",
                    label = div(icon("list-ol"), " Mostrar:"),
                    value = 20,
                    min = 5,
                    max = 100,
                    step = 5
                ...
                column(
                  ...
                  div(
                    style = "margin-top: 25px;",
                    actionButton(
                      "carregar_para_validacao",
                      label = div(
                        icon("sync-alt", style = "margin-right: 8px;"),
                        "Carregar Registros"
                      ),
                      class = "btn-primary btn-block",
                      style = "padding: 12px; border-radius: 25px; font-weight: 700;"
                    )
                  )
                ),
                column(
      ...
      # Adicionar nova aba no tabsetPanel principal
      tabPanel("🤖 Modelo Treinado", 
               icon = icon("brain"),
               
               fluidRow(
                 column(width = 12,
                        box(
                          title = "Treinamento e Validação do Modelo", 
                          status = "success", 
                          solidHeader = TRUE, 
                          width = 12,
                          ...
                          h4("🎯 Treinar Novo Modelo"),
                          p("Use os dados já classificados para treinar um modelo personalizado."),
                          ...
                          DT::dataTableOutput("metricas_modelo")
                        )
                 )
               )
      ),
      ...
      #===========================================================================
        # ABA: HISTÓRICO DE PROCESSAMENTOS (CORRIGIDA)
        #===========================================================================
        tabItem(
          tabName = "historico",
          # Header Hero
...
# Observers para capturar validações
observe({
  lapply(1:6, function(tipo)  {
    if(!is.null(values$resultados_lote)) {
      lapply(values$resultados_lote$nota_key, function(nota)  {
        ...
        observeEvent(input[[button_id]], {
          ...
          if(sucesso) {
            ...
            # Recarregar se filtro for "não validados"
            if(input$filtro_modelo_ml == "nao_validados") {
              # Trigger reload
              updateNumericInput(session, "limite_modelo_ml", 
                               value = input$limite_modelo_ml)
            }
          } else {
...
# Observer para ativar/desativar uso automático do modelo
observeEvent(input$usar_modelo_automatico, {
  
  if(input$usar_modelo_automatico) {
    ...
    # Ativar modelo
    if(exists("CONFIG_USUARIO")) {
      CONFIG_USUARIO$modelo_treinado$usar_automaticamente <- TRUE
    ...
    # Desativar modelo
    if(exists("CONFIG_USUARIO")) {
      CONFIG_USUARIO$modelo_treinado$usar_automaticamente <- FALSE
...
observeEvent(input$confirmar_reset_ml, {
  ...
  observeEvent(input$classificar_lote, {
    ...
    # ✅ USAR dados_cruzados (NÃO dados_preview)
  ...
  # ✅ DEBUG: Observer para monitorar mudanças em resultados_lote
  observe({
    cat("\n🔍 OBSERVER: Monitorando values$resultados_lote\n")
    cat("  - is.null:", is.null(values$resultados_lote), "\n")
    
    if(!is.null(values$resultados_lote)) {
      cat("  - nrow:", nrow(values$resultados_lote), "\n")
      cat("  - ncol:", ncol(values$resultados_lote), "\n")
      ...
      if("assunto_principal" %in% names(values$resultados_lote)) {
        ...
        print(head(values$resultados_lote$assunto_principal, 3))
      }
    }
    cat("\n")
  })
  ...
  output$total_historico_inline <- renderText({
    format(length(historico$processamentos), big.mark = ".")
  })
  ...
  output$total_validacoes_inline <- renderText({
    format(nrow(validacoes$dados), big.mark = ".")
  })
