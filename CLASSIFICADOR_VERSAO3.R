...
extrair_assunto_principal <- function(texto) {
  ...
  tryCatch({
    ...
    cat("🔗 URL da API:", url, "\n"),
      add_headers(`api-key` = OPENAI_CONFIG$api_key,
    ...
    withProgress(message = 'Classificando...', value = 0, {
      ...
      if(tem_comparacao) {
        ...
        HTML(paste0(
          ...
          # NOVO: Comparação visual de tipos
          ifelse(tem_comparacao,
            paste0(
              "<div style='display: grid; grid-template-columns: 1fr auto 1fr; gap: 15px; align-items: center; margin-bottom: 20px; padding: 15px; background: white; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);'>",
              
              # Tipo Anterior
              "<div style='text-align: center;'>",
              "<div style='color: #666; font-size: 12px; font-weight: bold; margin-bottom: 5px;'>TIPO ANTERIOR</div>",
              "<div style='font-size: 48px; color: #95a5a6; font-weight: bold;'>", tipo_anterior, "</div>",
              "<div style='font-size: 11px; color: #999; margin-top: 5px;'>", criticidade_anterior, "</div>",
              "</div>",
              
              # Seta
              "<div style='text-align: center; font-size: 36px; color: ", 
              ifelse(mudou, ifelse(nivel_novo > nivel_anterior, "#ff6b35", "#2e8b57"), "#1f4e79"), 
              ";'>",
              ifelse(mudou, ifelse(nivel_novo > nivel_anterior, "⬆️", "⬇️"), "✓"),
              "</div>",
              
              # Tipo Novo
              "<div style='text-align: center;'>",
              "<div style='color: #666; font-size: 12px; font-weight: bold; margin-bottom: 5px;'>TIPO SUGERIDO</div>",
              "<div style='font-size: 48px; color: #1f4e79; font-weight: bold;'>", resultado$tipo, "</div>",
              "<div style='font-size: 11px; color: #999; margin-top: 5px;'>", resultado$criticidade, "</div>",
              "</div>",
