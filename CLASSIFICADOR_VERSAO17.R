...
OPENAI_CONFIG <- list(
  base_url = Sys.getenv("AZURE_OPENAI_BASE_URL", unset = "https://apit.petrobras.com.br/ia/openai/v1/openai-azure/openai"),
  embedding_base_url = Sys.getenv("AZURE_OPENAI_BASE_URL_EMBEDDING", unset = "https://apit.petrobras.com.br/ia/openai/v1/openai-azure/openai"),
  api_key = Sys.getenv("OPENAI_API_KEY", unset = Sys.getenv("PETROBRAS_OPENAI_API_KEY", unset = "29d08064725944fcbc0b53e06f8807c5")),
  model = Sys.getenv("CHATGPT_MODEL", unset = "gpt-5-nano-petrobras"),
  api_version = Sys.getenv("OPENAI_API_VERSION", unset = "2024-10-21")
)

API_MODO_OFFLINE <- tolower(Sys.getenv("API_MODO_OFFLINE", unset = "false")) %in% c("1", "true", "sim", "yes")
API_DISPONIVEL <- NA
...
# Referências normativas e links de padrões (edite os links conforme sua base oficial)
PADROES_REFERENCIAS <- list(
  list(codigo = "SINPEP-9218", descricao = "Padrão SINPEP referência 9218", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=9218"),
  list(codigo = "SINPEP-20225", descricao = "Padrão SINPEP referência 20225", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=20225"),
  list(codigo = "SINPEP-26992", descricao = "Padrão SINPEP referência 26992", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=26992"),
  list(codigo = "SINPEP-34700", descricao = "Padrão SINPEP referência 34700", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=34700"),
  list(codigo = "SINPEP-39009", descricao = "Padrão SINPEP referência 39009", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=39009"),
  list(codigo = "SINPEP-25550", descricao = "Padrão SINPEP referência 25550", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=25550"),
  list(codigo = "SINPEP-28418", descricao = "Padrão SINPEP referência 28418", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=28418"),
  list(codigo = "SINPEP-35845", descricao = "Padrão SINPEP referência 35845", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=35845"),
  list(codigo = "SINPEP-17200", descricao = "Padrão SINPEP referência 17200", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=17200"),
  list(codigo = "SINPEP-30949", descricao = "Padrão SINPEP referência 30949", link = "https://sinpep.petrobras.com.br/SINPEP_Padrao_Web/PadraoEditar?PadraoId=30949")
)
...
if (!exists("MODELO_TREINADO_INTEGRADO")) {
  ...
  testar_acesso_cran <- function(mirror, metodo_download = "libcurl") {
    versao_r <- paste0(R.version$major, ".", strsplit(R.version$minor, "\\.")[[1]][1])
    urls_teste <- c(
      paste0(mirror, "/src/contrib/PACKAGES"),
      paste0(mirror, "/bin/windows/contrib/", versao_r, "/PACKAGES")
    )

    for (url_teste in urls_teste) {
      arquivo_tmp <- tempfile(fileext = ".txt")
      ok <- tryCatch({
        utils::download.file(
          url = url_teste,
          destfile = arquivo_tmp,
          method = metodo_download,
          quiet = TRUE,
          mode = "wb"
        )
        file.exists(arquivo_tmp) && file.info(arquivo_tmp)$size > 0
      }, warning = function(w) {
        FALSE
      }, error = function(e) {
        FALSE
  ...
  configurar_rede_e_repos <- function() {
    mirror_custom <- Sys.getenv("PETROBRAS_CRAN_MIRROR", unset = "")
    if (nchar(mirror_custom) == 0) mirror_custom <- Sys.getenv("R_CRAN_MIRROR", unset = "")
    if (nchar(mirror_custom) == 0) mirror_custom <- Sys.getenv("CRAN_MIRROR", unset = "")
    mirror_local <- Sys.getenv("PETROBRAS_LOCAL_CRAN", unset = "")

    if (nzchar(mirror_local) && dir.exists(mirror_local)) {
      caminho_local <- normalizePath(mirror_local, winslash = "/", mustWork = FALSE)
      mirror_local <- paste0("file:///", caminho_local)
    }

    repos_atuais <- getOption("repos")
    repos_cran_atual <- tryCatch(repos_atuais[["CRAN"]], error = function(e) "")

    mirrors_candidatos <- unique(Filter(nzchar, c(
      mirror_local,
      mirror_custom,
      repos_cran_atual,
    ...
    https_proxy <- Sys.getenv("HTTPS_PROXY", unset = Sys.getenv("https_proxy", unset = ""))
    http_proxy <- Sys.getenv("HTTP_PROXY", unset = Sys.getenv("http_proxy", unset = ""))
    proxy_generico <- Sys.getenv("PETROBRAS_PROXY", unset = "")

    if (nchar(https_proxy) == 0 && nchar(proxy_generico) > 0) https_proxy <- proxy_generico
    if (nchar(http_proxy) == 0 && nchar(proxy_generico) > 0) http_proxy <- proxy_generico

    if (nchar(https_proxy) > 0) {
      Sys.setenv(https_proxy = https_proxy, HTTPS_PROXY = https_proxy)
      cat("🔐 HTTPS proxy configurado via ambiente\n")
    }
    if (nchar(http_proxy) > 0) {
      Sys.setenv(http_proxy = http_proxy, HTTP_PROXY = http_proxy)
      cat("🔐 HTTP proxy configurado via ambiente\n")
    }

    metodos_download <- if (.Platform$OS.type == "windows") c("wininet", "libcurl") else c("libcurl")

    for (metodo in metodos_download) {
      for (mirror in mirrors_candidatos) {
        if (isTRUE(testar_acesso_cran(mirror, metodo_download = metodo))) {
          options(repos = c(CRAN = mirror))
          options(download.file.method = metodo)
          cat("🌐 Mirror CRAN ativo:", mirror, "| método:", metodo, "\n")
          return(list(
            cran_ok = TRUE,
            mirror_ativo = mirror,
            metodo_ativo = metodo,
            mirrors_testados = mirrors_candidatos,
            metodos_testados = metodos_download
          ))
        }
      }
    }

    if (length(mirrors_candidatos) > 0) {
      options(repos = c(CRAN = mirrors_candidatos[1]))
    }
    if (length(metodos_download) > 0) {
      options(download.file.method = metodos_download[1])
    }

    cat("❌ Não foi possível acessar índices do CRAN com os mirrors/métodos testados.\n")
    cat("   → Verifique proxy corporativo ou defina PETROBRAS_CRAN_MIRROR com mirror interno.\n")

    list(
      cran_ok = FALSE,
      mirror_ativo = getOption("repos")[["CRAN"]],
      metodo_ativo = getOption("download.file.method"),
      mirrors_testados = mirrors_candidatos,
      metodos_testados = metodos_download
    )
  }

  configurar_bibliotecas_locais <- function() {
    libs_custom <- Sys.getenv("PETROBRAS_R_LIBS", unset = "")
    libs_usuario <- Sys.getenv("R_LIBS_USER", unset = "")

    candidatos <- unique(Filter(nzchar, c(
      unlist(strsplit(libs_custom, ";", fixed = TRUE)),
      unlist(strsplit(libs_usuario, ";", fixed = TRUE)),
      Sys.getenv("R_USER", unset = "")
    )))

    if (length(candidatos) > 0) {
      candidatos <- unique(gsub("\\\\", "/", trimws(candidatos)))
      candidatos_validos <- candidatos[dir.exists(candidatos)]

      if (length(candidatos_validos) > 0) {
        .libPaths(unique(c(candidatos_validos, .libPaths())))
        cat("📚 Bibliotecas R adicionais carregadas:\n")
        for (lib in candidatos_validos) cat("   -", lib, "\n")
      }
    }
  }

  instalar_se_faltar <- function(pacotes, obrigatorio = TRUE) {
    configurar_bibliotecas_locais()

    modo_sem_instalacao <- tolower(Sys.getenv("PETROBRAS_SKIP_INSTALL_PACKAGES", unset = "false")) %in% c("1", "true", "sim", "yes")
    config_rede <- configurar_rede_e_repos()
    ...
    cat("📦 Pacotes ausentes:", paste(faltantes, collapse = ", "), "\n")

    if (isTRUE(modo_sem_instalacao)) {
      cat("ℹ️ Modo sem instalação ativado (PETROBRAS_SKIP_INSTALL_PACKAGES=TRUE).\n")
      if (obrigatorio) {
        stop(paste0(
          "Pacotes obrigatórios não encontrados nas bibliotecas ativas (.libPaths): ",
          paste(faltantes, collapse = ", "),
          ". Ajuste PETROBRAS_R_LIBS/R_LIBS_USER para a pasta correta ou desative PETROBRAS_SKIP_INSTALL_PACKAGES para permitir instalação automática."
        ))
      }
      return(faltantes)
    }

    if (!isTRUE(config_rede$cran_ok)) {
      if (obrigatorio) {
        stop(paste0(
          "Pacotes obrigatórios ausentes e sem acesso ao CRAN. ",
          "Pacotes faltantes: ", paste(faltantes, collapse = ", "), ". ",
          "Defina PETROBRAS_CRAN_MIRROR (URL corporativa) ou PETROBRAS_LOCAL_CRAN (pasta local com repositório). ",
          "Se necessário, configure HTTPS_PROXY/HTTP_PROXY."
        ))
      }

      cat("⚠️ Sem acesso ao CRAN; instalação automática ignorada para pacotes opcionais.\n")
      return(faltantes)
    }

    for (pkg in faltantes) {
      cat("   ↳ Tentando instalar:", pkg, "...\n")
      tryCatch({
        install.packages(
          pkg,
          dependencies = TRUE,
        ...
        cat("   ⚠️ Falha ao instalar", pkg, ":", e$message, "\n")
      })
    }

    ainda_faltantes <- pacotes[!vapply(pacotes, requireNamespace, logical(1), quietly = TRUE)]

    if (obrigatorio && length(ainda_faltantes) > 0) {
      if (!isTRUE(config_rede$cran_ok)) {
        stop(paste0(
          "Pacotes obrigatórios ausentes e sem acesso ao CRAN. ",
          "Mirrors testados: ", paste(config_rede$mirrors_testados, collapse = ", "), ". ",
          "Métodos testados: ", paste(config_rede$metodos_testados, collapse = ", "), ". ",
          "Defina PETROBRAS_CRAN_MIRROR (mirror interno) e/ou HTTPS_PROXY/HTTP_PROXY."
        ))
      }

      stop(paste0(
        "Pacotes obrigatórios ausentes após tentativa de instalação: ",
        paste(ainda_faltantes, collapse = ", "),
        "."
      ))
    }

    return(ainda_faltantes)
  }

  pacotes_base <- c("shiny", "shinydashboard", "DT", "ggplot2", "dplyr", "httr", "jsonlite", "rlang", "tidyr")
  faltantes_base <- instalar_se_faltar(pacotes_base, obrigatorio = TRUE)

  invisible(lapply(pacotes_base[!pacotes_base %in% faltantes_base], function(pkg) {
    suppressPackageStartupMessages(library(pkg, character.only = TRUE))
  }))

  if (!requireNamespace("readxl", quietly = TRUE)) {
    cat("ℹ️ Pacote opcional 'readxl' não encontrado. Importação de arquivos Excel (.xlsx/.xls) ficará indisponível; use CSV ou instale o pacote quando possível.\n")
  }

  bibliotecas_ml <- c("randomForest", "caret", "tm", "e1071")
  bibliotecas_ml_faltantes <- instalar_se_faltar(bibliotecas_ml, obrigatorio = FALSE)

  MODELO_ML_DISPONIVEL <- length(bibliotecas_ml_faltantes) == 0
  if (isTRUE(MODELO_ML_DISPONIVEL)) {
    cat("✅ Bibliotecas de Machine Learning disponíveis\n")
  } else {
    cat("⚠️ Bibliotecas de ML ausentes:", paste(bibliotecas_ml_faltantes, collapse = ", "), "\n")
    cat("⚠️ Modo fallback ativado: classificação por dicionário/API sem treino ML local\n")
  }
...
predizer_com_modelo <- function(texto) {
  ...
  tryCatch({
    ...
    # Forçar tipo válido (1-6)
    if(is.na(tipo_predito) || tipo_predito < 1 || tipo_predito > 6) tipo_predito <- 3
    # Calcular confiança
    ...
    categoria_final <- if(!is.null(tipo_info) && !is.null(tipo_info$categoria_principal) && tipo_info$categoria_principal != "INDEFINIDO") tipo_info$categoria_principal else "PROBLEMAS_COMUNS"
    return(list(
      sucesso = TRUE,
      tipo = tipo_predito,
...
enviar_chat_com_fallback_temperatura <- function(url, body, api_key, timeout_segundos = 30, log_fn = NULL) {
  log_mensagem <- function(msg) {
  ...
  executar_requisicao <- function(body_req) {
    POST(
      url = url,
      add_headers(
        `api-key` = api_key,
      ...
      body = toJSON(body_req, auto_unbox = TRUE),
      encode = "json",
      timeout(timeout_segundos)
    )
  }

  response <- executar_requisicao(body)

  temperatura_solicitada <- body$temperature
  if (!is.null(temperatura_solicitada) && !isTRUE(all.equal(temperatura_solicitada, 1))) {
    status_http <- tryCatch(status_code(response), error = function(e) NA_integer_)

    if (identical(status_http, 400L)) {
      texto_erro <- tryCatch(httr::content(response, "text", encoding = "UTF-8"), error = function(e) "")
      temperatura_nao_suportada <- grepl("temperature", texto_erro, ignore.case = TRUE) &&
        grepl("unsupported|default \\(1\\)|does not support", texto_erro, ignore.case = TRUE)

      if (isTRUE(temperatura_nao_suportada)) {
        log_mensagem(paste0(
          "ℹ️ Modelo não aceitou temperature=", temperatura_solicitada,
          ". Reenviando automaticamente com temperature=1."
        ))

        body_retry <- body
        body_retry$temperature <- 1
        response <- executar_requisicao(body_retry)
      }
    }
  }
...
gerar_insights_estatisticos <- function(metricas_dados)  {
  ...
  tryCatch({
    ...
    response <- enviar_chat_com_fallback_temperatura(
      url = url,
      body = body,
...
atualizar_status_api_disponivel <- function(force_check = FALSE) {
  if (!force_check && exists("API_DISPONIVEL") && !is.na(API_DISPONIVEL)) {
    return(list(disponivel = API_DISPONIVEL, motivo = ifelse(API_DISPONIVEL, "ok", "indisponivel_cache")))
  }

  if (isTRUE(API_MODO_OFFLINE)) {
    API_DISPONIVEL <<- FALSE
    return(list(disponivel = FALSE, motivo = "modo_offline_ativo"))
  }

  if (is.null(OPENAI_CONFIG$api_key) || !nzchar(trimws(OPENAI_CONFIG$api_key))) {
    API_DISPONIVEL <<- FALSE
    return(list(disponivel = FALSE, motivo = "api_key_nao_configurada"))
  }

  if (!requireNamespace("httr", quietly = TRUE)) {
    API_DISPONIVEL <<- FALSE
    return(list(disponivel = FALSE, motivo = "pacote_httr_indisponivel"))
  }

  status_probe <- tryCatch({
    resp <- httr::HEAD(OPENAI_CONFIG$base_url, httr::timeout(4))
    httr::status_code(resp)
  }, error = function(e) {
    NA_integer_
  })

  API_DISPONIVEL <<- !is.na(status_probe)
  if (isTRUE(API_DISPONIVEL)) {
    return(list(disponivel = TRUE, motivo = paste0("probe_http_", status_probe)))
  }

  return(list(disponivel = FALSE, motivo = "sem_conectividade_api"))
}
...
classificar_com_openai <- function(texto)  {
  ...
  status_api <- atualizar_status_api_disponivel(force_check = FALSE)
  if (!isTRUE(status_api$disponivel)) {
    log_dual(paste("⚠️ API indisponível:", status_api$motivo, "| fallback para dicionário"))
    resultado <- classificar_por_dicionario(texto, DICIONARIOS_SAP)
    resultado$erro <- TRUE
    resultado$metodo <- "FALLBACK_OFFLINE"
    return(resultado)
  }
  ...
  construir_contexto_padroes_api <- function(dicionarios = DICIONARIOS_SAP) {
    tipos <- paste0("tipo_", 1:6)
    linhas <- vapply(tipos, function(tipo_key) {
      bloco <- dicionarios[[tipo_key]]
      if (is.null(bloco)) return("")
      palavras <- bloco$palavras_chave
      if (is.null(palavras) || length(palavras) == 0) palavras <- character(0)
      palavras <- unique(as.character(palavras))
      palavras_exemplo <- paste(head(palavras, 12), collapse = ", ")
      paste0(
        toupper(tipo_key), " | categoria=", bloco$categoria_principal,
        " | criticidade=", bloco$criticidade,
        " | descricao=", bloco$descricao,
        " | palavras_chave=[", palavras_exemplo, "]"
      )
    }, character(1))
    paste(linhas[nzchar(linhas)], collapse = "\n")
  }

  construir_links_padroes_api <- function(referencias = PADROES_REFERENCIAS) {
    if (is.null(referencias) || length(referencias) == 0) {
      return("Nenhuma referência normativa cadastrada.")
    }

    linhas <- vapply(referencias, function(ref) {
      codigo <- if (!is.null(ref$codigo) && nzchar(ref$codigo)) ref$codigo else "SEM_CODIGO"
      descricao <- if (!is.null(ref$descricao) && nzchar(ref$descricao)) ref$descricao else "Sem descrição"
      link <- if (!is.null(ref$link) && nzchar(ref$link)) ref$link else "LINK_NAO_INFORMADO"
      paste0(codigo, " | ", descricao, " | ", link)
    }, character(1))

    paste(linhas, collapse = "\n")
  }

  contexto_padroes <- construir_contexto_padroes_api(DICIONARIOS_SAP)
  contexto_links <- construir_links_padroes_api(PADROES_REFERENCIAS)
  
  prompt <- paste0(
    "Você é um especialista em classificação SAP de manutenção da Petrobras.\n\n",
    "Use prioritariamente a biblioteca de padrões enviada (DICIONARIOS_SAP) como referência normativa.\n",
    "Considere também os links de padrões oficiais como fonte de apoio para desambiguação.\n",
    "Em caso de ambiguidade, escolha o tipo com maior aderência às palavras-chave dos padrões.\n\n",
    "REFERÊNCIAS NORMATIVAS (LINKS):\n",
    contexto_links,
    "\n\n",
    "BIBLIOTECA DE PADRÕES (LIBS):\n",
    contexto_padroes,
    "\n\n",
  ...
  tryCatch({
    ...
    log_dual(paste("🔑 API Key:", ifelse(nzchar(trimws(OPENAI_CONFIG$api_key)), "CONFIGURADA (oculta)", "NÃO CONFIGURADA")))
...
teste_smoke_classificacao <- function(config = CONFIG_USUARIO) {
  cfg <- config
  if (is.null(cfg$usar_api)) cfg$usar_api <- TRUE
  if (is.null(cfg$usar_dicionario)) cfg$usar_dicionario <- TRUE
  if (is.null(cfg$usar_modelo_treinado)) cfg$usar_modelo_treinado <- TRUE
  ...
  comparativo <- do.call(rbind, lapply(seq_len(nrow(casos)), function(i) {
    txt <- casos$texto[i]
    esperado <- casos$esperado[i]

    res_api <- tryCatch(classificar_com_openai(txt), error = function(e) list(erro = TRUE, mensagem = e$message))
    res_hib <- tryCatch(classificar_hibrido_completo(txt, cfg), error = function(e) list(erro = TRUE, mensagem = e$message))

    api_tipo <- if (!isTRUE(res_api$erro)) res_api$tipo else NA
    hib_tipo <- if (!isTRUE(res_hib$erro)) res_hib$tipo else NA

    data.frame(
      caso = casos$caso[i],
      esperado = esperado,
      api_tipo = api_tipo,
      api_conf = if (!isTRUE(res_api$erro)) res_api$confianca else NA,
      api_status = if (!is.na(api_tipo) && api_tipo == esperado) "OK" else "ALERTA",
      hib_tipo = hib_tipo,
      hib_conf = if (!isTRUE(res_hib$erro)) res_hib$confianca else NA,
      hib_metodo = if (!isTRUE(res_hib$erro) && !is.null(res_hib$metodo)) res_hib$metodo else NA,
      hib_status = if (!is.na(hib_tipo) && hib_tipo == esperado) "OK" else "ALERTA",
      stringsAsFactors = FALSE
    )
  }))
  ...
  print(comparativo)
  cat("\nResumo Smoke Test:\n")
  cat("API OK:", sum(comparativo$api_status == "OK", na.rm = TRUE), "/", nrow(comparativo), "\n")
  cat("HIBRIDO OK:", sum(comparativo$hib_status == "OK", na.rm = TRUE), "/", nrow(comparativo), "\n")

  invisible(comparativo)
}
...
classificar_hibrido_completo <- function(texto, config) {
  ...
  # Se nenhum método funcionou, retornar fallback seguro
  if(metodos_disponiveis == 0) {
    cat("   ❌ ERRO: Nenhum método de classificação funcionou! Fallback seguro ativado.\n")
    return(list(
      tipo = 3,
      categoria = "PROBLEMAS_COMUNS",
      criticidade = "MEDIA",
      confianca = 50,
      descricao = "Classificação padrão: nenhum método disponível.",
      resumo = "Fallback seguro: Todos os métodos falharam.",
      metodo = "ERRO_HIBRIDO_SEGURO"
    ))
  }
  ...
  # Determinar vencedor
  if(sum(votos) == 0) {
    # Fallback: procurar o primeiro resultado realmente válido (tipo 1-6)
    cat("   ⚠️ Nenhum voto válido, usando fallback reforçado\n")
    resultado_final <- NULL
    # Procurar por tipo válido (1-6) em ordem de prioridade: dicionario, api, ml
    for (fonte in c("dicionario", "api", "ml")) {
      res <- resultados[[fonte]]
      if (!is.null(res) && !is.null(res$tipo) && !is.na(res$tipo) && res$tipo >= 1 && res$tipo <= 6) {
        resultado_final <- res
        resultado_final$metodo <- paste0(toupper(fonte), "_FALLBACK")
        break
      }
    }
    if (!is.null(resultado_final)) {
      # Garantir que todos os campos estejam preenchidos e nunca retornar tipo=0 ou categoria=INDEFINIDO
      if(is.null(resultado_final$tipo) || is.na(resultado_final$tipo) || resultado_final$tipo < 1 || resultado_final$tipo > 6) resultado_final$tipo <- 3
      if(is.null(resultado_final$categoria) || is.na(resultado_final$categoria) || resultado_final$categoria == "INDEFINIDO") resultado_final$categoria <- "PROBLEMAS_COMUNS"
      if(is.null(resultado_final$criticidade) || is.na(resultado_final$criticidade)) resultado_final$criticidade <- "MEDIA"
      if(is.null(resultado_final$confianca) || is.na(resultado_final$confianca)) resultado_final$confianca <- 50
      if(is.null(resultado_final$descricao) || is.na(resultado_final$descricao)) resultado_final$descricao <- "Classificação padrão: fallback."
      if(is.null(resultado_final$resumo) || is.na(resultado_final$resumo)) resultado_final$resumo <- "Fallback seguro."
      return(resultado_final)
    } else {
      # Fallback seguro: nenhum método retornou tipo válido
      return(list(
        tipo = 3,
        categoria = "PROBLEMAS_COMUNS",
        criticidade = "MEDIA",
        confianca = 50,
        descricao = "Classificação padrão: nenhum método disponível.",
        resumo = "Fallback seguro: Nenhum voto válido.",
        metodo = "ERRO_FALLBACK_SEGURO"
      ))
    }
  }
  ...
  # Montar resultado final usando o primeiro resultado disponível como base, mas garantir tipo válido
  resultado_final <- NULL
  for (fonte in c("dicionario", "api", "ml")) {
    res <- resultados[[fonte]]
    if (!is.null(res) && !is.null(res$tipo) && !is.na(res$tipo) && res$tipo >= 1 && res$tipo <= 6) {
  ...
  if (is.null(resultado_final)) {
    # Isso nunca deve acontecer devido à verificação anterior, mas reforço
    return(list(
      tipo = tipo_final, categoria = "PROBLEMAS_COMUNS", criticidade = "MEDIA",
  ...
  # Corrigir categoria: buscar do método vencedor
  categoria_vencedora <- NULL
  for (fonte in c("dicionario", "api", "ml")) {
    res <- resultados[[fonte]]
    if (!is.null(res) && !is.null(res$tipo) && res$tipo == tipo_final && !is.null(res$categoria) && res$categoria != "INDEFINIDO") {
      categoria_vencedora <- res$categoria
      break
    }
  }
  if (!is.null(categoria_vencedora)) {
    resultado_final$categoria <- categoria_vencedora
  } else {
    resultado_final$categoria <- "PROBLEMAS_COMUNS"
  }
  if(is.null(resultado_final$criticidade) || is.na(resultado_final$criticidade)) resultado_final$criticidade <- "MEDIA"
  if(is.null(resultado_final$descricao) || is.na(resultado_final$descricao)) resultado_final$descricao <- "Tipo determinado por votação"
  if(is.null(resultado_final$resumo) || is.na(resultado_final$resumo)) resultado_final$resumo <- "Votação híbrida"
  ...
  # Retornar apenas os campos principais, nunca uma lista aninhada
  return(list(
    tipo = resultado_final$tipo,
    categoria = resultado_final$categoria,
    criticidade = resultado_final$criticidade,
    confianca = resultado_final$confianca,
    descricao = resultado_final$descricao,
    resumo = resultado_final$resumo,
    metodo = resultado_final$metodo
  ))
}
                    ...
                    div(
                      ...
                      tags$code(
                        style = "background: rgba(255,255,255,0.6); padding: 5px 10px; 
                           border-radius: 6px; font-size: 11px; display: inline-block;
                           margin-top: 5px;",
                        ifelse(nzchar(trimws(OPENAI_CONFIG$api_key)), "CONFIGURADA (oculta)", "NÃO CONFIGURADA")
                      )
                    )
                  )
                ),
...
server <- function(input, output, session)  {
  ...
  # Função vazia para compatibilidade (sistema de log foi removido)
  adicionar_log <- function(mensagem, prefixo = "") {
          ...
          div(
            ...
            div(
              style = "background: #f8f9fa; padding: 15px; border-radius: 8px; 
                   margin-bottom: 20px; border-left: 4px solid #667eea; max-height: 150px; overflow-y: auto;",
              p(style = "margin: 0; font-size: 13px; color: #666; line-height: 1.8;",
                substr(registro$texto_completo, 1, 400), 
                ifelse(nchar(registro$texto_completo) > 400, "...", ""))
            ),
  ...
  observeEvent(input$arquivo_ordens, {
    ...
    tryCatch({
      ...
      if(ext == "csv") {
        ...
        if (!requireNamespace("readxl", quietly = TRUE)) {
          stop("Para importar arquivos Excel, instale o pacote opcional 'readxl' ou utilize CSV.")
        }
        values$dados_ordens <- readxl::read_excel(input$arquivo_ordens$datapath)
      }
  ...
  observeEvent(input$arquivo_textos, {
    ...
    tryCatch({
      ...
      if(ext == "csv") {
        ...
        values$dados_textos <- readxl::read_excel(input$arquivo_textos$datapath)
      }
      ...
      withProgress(message = 'Classificando em lote...', value = 0, {
        for (i in 1:total) {
          ...
          classificacao <- tryCatch({
            ...
            # ✅ VALIDAÇÃO: NÃO permitir tipos 5 ou 6 se campo não tem hífen
            ...
            if (!tem_hifen && !is.na(res$tipo) && res$tipo %in% c(5, 6)) {
              ...
              res$confianca <- 0
              res$descricao <- "Classificação não permitida: campo sem hífen"
              res$resumo <- "Tipo 5/6 rejeitado por ausência de hífen"
            }
            
            res
          }, error = function(e) {
            list(
              tipo = NA, categoria = "INDEFINIDO", criticidade = "MEDIA",
              confianca = 0, descricao = "Classificação padrão (erro interno)",
              resumo = "Classificação padrão", metodo = input$metodo_classificacao
            )
          })
          # LOG DE DIAGNÓSTICO: registrar tipo e categoria retornados pela classificação
          cat(sprintf("[DEBUG] Linha %d: tipo retornado = %s, categoria = %s\n", i, as.character(classificacao$tipo), as.character(classificacao$categoria)))
          # LÓGICA V17: Nunca retornar tipo_novo = 0 ou categoria = INDEFINIDO se houver qualquer voto válido
          tipo_final <- classificacao$tipo
          categoria_final <- classificacao$categoria
          # Se tipo_final é NA ou 0, tentar fallback igual V17
          if (is.null(tipo_final) || is.na(tipo_final) || tipo_final == 0) {
            # Fallback: usar o primeiro método disponível
            if (!is.null(classificacao$dicionario) && !is.null(classificacao$dicionario$tipo) && !is.na(classificacao$dicionario$tipo)) {
              tipo_final <- classificacao$dicionario$tipo
              categoria_final <- classificacao$dicionario$categoria
            } else if (!is.null(classificacao$api) && !is.null(classificacao$api$tipo) && !is.na(classificacao$api$tipo)) {
              tipo_final <- classificacao$api$tipo
              categoria_final <- classificacao$api$categoria
            } else if (!is.null(classificacao$ml) && !is.null(classificacao$ml$tipo) && !is.na(classificacao$ml$tipo)) {
              tipo_final <- classificacao$ml$tipo
              categoria_final <- classificacao$ml$categoria
            } else {
              tipo_final <- NA
              categoria_final <- "INDEFINIDO"
            }
          }
          # Se categoria ficou indefinida mas tipo é válido, corrigir para PROBLEMAS_COMUNS
          if ((is.null(categoria_final) || categoria_final == "INDEFINIDO" || is.na(categoria_final)) && !is.null(tipo_final) && !is.na(tipo_final) && tipo_final > 0) {
            categoria_final <- "PROBLEMAS_COMUNS"
          }
          resultados$tipo_novo[i]      <- tipo_final
          resultados$categoria[i]      <- categoria_final
          ...
          # CORREÇÃO FINAL: nunca permitir tipo_novo = NA/INDEFINIDO
          if (is.null(resultados$tipo_novo[i]) || is.na(resultados$tipo_novo[i]) || resultados$tipo_novo[i] == 0) {
            resultados$tipo_novo[i] <- 3
          }
          if (is.null(resultados$categoria[i]) || is.na(resultados$categoria[i]) || resultados$categoria[i] == "INDEFINIDO") {
            resultados$categoria[i] <- "PROBLEMAS_COMUNS"
          }
          cat(sprintf("[DEBUG] Linha %d: tipo_novo atribuído = %s, categoria atribuída = %s\n", i, as.character(resultados$tipo_novo[i]), as.character(resultados$categoria[i])))
          
          # Status de conformidade
          tipo_antigo <- resultados$tipo_intervencao_antigo[i]
          # Ajuste: status só é 'SEM_REFERENCIA' se algum dos tipos for NA ou inválido
          tipo_novo <- resultados$tipo_novo[i]
          if (!is.na(tipo_antigo) && tipo_antigo > 0 && !is.na(tipo_novo) && tipo_novo > 0) {
            resultados$status_conformidade[i] <- ifelse(
              tipo_antigo == tipo_novo,
              "CONFORME",
              "DIVERGENTE"
            )
          } else {
  ...
  observeEvent(input$salvar_config, {
    ...
    if (nzchar(trimws(input$config_api_key))) {
  ...
  output$status_api <- renderText({
    paste(
      ...
      paste("Chave:", ifelse(nzchar(trimws(OPENAI_CONFIG$api_key)), "CONFIGURADA (oculta)", "NÃO CONFIGURADA")),
      sep = "\n"
    )
  })
  ...
  observeEvent(input$testar_api, {
    ...
    # Atualizar chave a partir do ambiente, caso a configuração em memória esteja vazia
    if (is.null(OPENAI_CONFIG$api_key) || !nzchar(trimws(OPENAI_CONFIG$api_key))) {
      OPENAI_CONFIG$api_key <<- Sys.getenv("OPENAI_API_KEY", unset = Sys.getenv("PETROBRAS_OPENAI_API_KEY", unset = ""))
    }

    # Gate explícito: não tentar chamada se a chave não estiver configurada
    if (is.null(OPENAI_CONFIG$api_key) || !nzchar(trimws(OPENAI_CONFIG$api_key))) {
      cat("⚠️ API Key não configurada. Teste abortado antes da chamada.\n")
      output$resultado_teste_api <- renderUI({
        HTML("<div style='padding: 15px; background: #fff3cd; border-radius: 8px; border-left: 4px solid #ffc107;'>
          <strong style='color: #856404;'>⚠️ API KEY NÃO CONFIGURADA</strong>
          <p style='color: #856404; margin: 5px 0 0 0;'>Defina OPENAI_API_KEY (ou PETROBRAS_OPENAI_API_KEY) e tente novamente.</p>
          </div>")
      })
      cat(strrep("═", 60), "\n\n")
    ...
    tryCatch({
      ...
      cat("🔗 URL:", url, "\n")
      cat("🔑 API Key:", ifelse(nzchar(trimws(OPENAI_CONFIG$api_key)), "CONFIGURADA (oculta)", "NÃO CONFIGURADA"), "\n")
