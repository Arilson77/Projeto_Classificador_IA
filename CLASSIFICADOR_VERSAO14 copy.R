...
#=============================================================================
# FUNÇÃO CORRIGIDA: SALVAR VALIDAÇÃO ML (v2 - Mais Robusta)
#=============================================================================

salvar_validacao_ml <- function(registro_id, tipo_validado, feedback = "boa", dados_resultados = NULL)  {
  ...
  tryCatch({
    ...
    # OPÇÃO 1: Usar dados passados como parâmetro
    if (!is.null(dados_resultados)) {
      dados_encontrados <- dados_resultados
      cat("✓ Usando dados passados como parâmetro\n")
    }
    
    # OPÇÃO 2: Procurar em values$resultados_lote
    if (is.null(dados_encontrados) && exists("values", inherits = TRUE)) {
      if (!is.null(values$resultados_lote) && nrow(values$resultados_lote) > 0) {
        dados_encontrados <- values$resultados_lote
        cat("✓ Usando dados de values$resultados_lote (", nrow(dados_encontrados), " linhas)\n")
      }
    }
    
    # OPÇÃO 3: Procurar em validacoes_modelo$dados_temporarios
    if (is.null(dados_encontrados) && exists("validacoes_modelo", inherits = TRUE)) {
      if (!is.null(validacoes_modelo$dados_temporarios) && nrow(validacoes_modelo$dados_temporarios) > 0) {
        dados_encontrados <- validacoes_modelo$dados_temporarios
        cat("✓ Usando dados de validacoes_modelo$dados_temporarios\n")
      }
    }
    
    # Se ainda não encontrou, retornar erro
    if (is.null(dados_encontrados) || nrow(dados_encontrados) == 0) {
      cat("⚠️ Dados não encontrados em nenhuma fonte\n")
      return(FALSE)
    }
    
    # Garantir que nota_key seja string
    if ("nota_key" %in% names(dados_encontrados)) {
      dados_encontrados$nota_key <- as.character(dados_encontrados$nota_key)
    }
    
    # Debug: mostrar o que está procurando
    cat("🔍 Procurando registro ID:", registro_id, "\n")
    if ("nota_key" %in% names(dados_encontrados)) {
      cat("   Primeiras 5 notas:", paste(head(dados_encontrados$nota_key, 5), collapse = ", "), "\n")
    }
    
    # Filtrar por nota_key ou por índice
    registro <- NULL
    if ("nota_key" %in% names(dados_encontrados)) {
      registro <- dados_encontrados %>%
        dplyr::filter(nota_key == registro_id)
    } else if ("índice" %in% names(dados_encontrados)) {
      registro <- dados_encontrados %>%
        dplyr::filter(índice == as.numeric(registro_id))
    } else {
      # Tentar conversão de número
      idx <- suppressWarnings(as.numeric(registro_id))
      if (!is.na(idx) && idx >= 1 && idx <= nrow(dados_encontrados)) {
        registro <- dados_encontrados[idx, ]
      }
    }
    
    # Se encontrou o registro
    if (!is.null(registro) && (is.data.frame(registro) && nrow(registro) > 0 || !is.data.frame(registro))) {
      
      # Extrair informações (suportar list ou data.frame)
      if (is.data.frame(registro)) {
        texto_original <- as.character(registro$texto_completo[1])
        tipo_antigo <- as.integer(ifelse(is.null(registro$tipo_intervencao_antigo[1]) || is.na(registro$tipo_intervencao_antigo[1]), 
                                         NA, registro$tipo_intervencao_antigo[1]))
        tipo_ia <- as.integer(registro$tipo_novo[1])
        assunto_original <- as.character(ifelse(is.null(registro$assunto_principal[1]), "", registro$assunto_principal[1]))
        confianca <- as.numeric(registro$confianca[1])
      } else {
        texto_original <- as.character(registro$texto_completo)
        tipo_antigo <- as.integer(ifelse(is.null(registro$tipo_intervencao_antigo) || is.na(registro$tipo_intervencao_antigo), NA, registro$tipo_intervencao_antigo))
        tipo_ia <- as.integer(registro$tipo_novo)
        assunto_original <- as.character(ifelse(is.null(registro$assunto_principal), "", registro$assunto_principal))
        confianca <- as.numeric(registro$confianca)
      }
      ...
      nova_validacao <- data.frame(
        ...
        tipo_original = tipo_antigo,
        tipo_ia = tipo_ia,
        tipo_validado = tipo_validado,
        assunto_original = assunto_original,
        assunto_validado = assunto_original,
        confianca = confianca,
        feedback_qualidade = feedback,
        timestamp = Sys.time(),
        usuario = ifelse(exists("input$usuario", inherits = TRUE), input$usuario, "sistema"),
      ...
      if (is.null(validacoes_modelo$dados) || nrow(validacoes_modelo$dados) == 0) {
        validacoes_modelo$dados <- nova_validacao
      } else {
        # Remover validação anterior se existir (evitar duplicatas)
      ...
      # Salvar em disco
      tryCatch({
        saveRDS(validacoes_modelo$dados, file.path("dados_modelo_treinado", paste0("validacoes_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".rds")))
        cat("💾 Arquivo salvo em disco\n")
      }, error = function(e) {
        cat("⚠️ Erro ao salvar em disco:", as.character(e), "\n")
      })
    ...
    cat("⚠️ Registro não encontrado! ID procurado:", registro_id, "\n")
    if ("nota_key" %in% names(dados_encontrados)) {
      cat("   Notas disponíveis:", paste(head(dados_encontrados$nota_key, 10), collapse = ", "), "\n")
    }
    return(FALSE)
...
salvar_validacao_ml_incremental <- function(registro_id, tipo_validado, feedback = "boa") {
  # Salvar validação (existente)
  sucesso <- salvar_validacao_ml(registro_id, tipo_validado, feedback)
  if (sucesso) {
...
classificar_hibrido_completo <- function(texto, config) {
  ...
  # ========================================================================
  # ETAPA 1: DICIONÁRIO (sempre disponível como fallback)
  # ========================================================================
  ...
  # ========================================================================
  # ADICIONAR METADADOS DE DIAGNÓSTICO
  # ========================================================================
...
classificar_hibrido <- function(texto, config) {
  ...
  if(resultado_api$erro) {
    resultado_dicionario$metodo <- "DICIONARIO_API_FALLBACK"
    resultado_dicionario$resumo <- paste0(
      "⚠️ API indisponível. Usando apenas dicionário. ",
      "Confiança: ", resultado_dicionario$confianca, "%"
    )
  ...
  if(resultado_dicionario$tipo == resultado_api$tipo) {
    ...
    resultado_dicionario$confianca <- min(100, (resultado_dicionario$confianca + resultado_api$confianca) / 2)
    resultado_dicionario$metodo <- "HIBRIDO_DICIONARIO_API"
    resultado_dicionario$resumo <- paste0(
      "✅ Dicionário e API concordam. ",
      "Tipo ", resultado_dicionario$tipo, 
      " - Criticidade: ", resultado_dicionario$criticidade,
      " - Confiança: ", round(resultado_dicionario$confianca), "%"
    )
  } else {
    # Divergem - usar maior confiança
    if(resultado_dicionario$confianca >= resultado_api$confianca) {
      resultado_dicionario$metodo <- "HIBRIDO_DICIONARIO_PREDOMINANTE"
      resultado_dicionario$resumo <- paste0(
        "⚠️ Divergência: API sugeriu tipo ", resultado_api$tipo, ". ",
        "Usando dicionário (confiança: ", resultado_dicionario$confianca, "% vs ", resultado_api$confianca, "%)"
      )
    } else {
      resultado_dicionario$tipo <- resultado_api$tipo
      resultado_dicionario$categoria <- resultado_api$categoria
      resultado_dicionario$criticidade <- resultado_api$criticidade
      resultado_dicionario$confianca <- resultado_api$confianca
      resultado_dicionario$metodo <- "HIBRIDO_API_PREDOMINANTE"
      resultado_dicionario$resumo <- paste0(
        "⚠️ Divergência: Dicionário sugeriu tipo ", 
        classificar_por_dicionario(texto, config$dicionarios)$tipo, ". ",
        "Usando API (confiança: ", resultado_api$confianca, "% vs ", 
        classificar_por_dicionario(texto, config$dicionarios)$confianca, "%)"
      )
    }
  }
...
# Observers para validações - CENTRALIZADO (evita duplicatas)
# Usar shinyjs para interceptar TODOS os cliques de validação em um único observeEvent

observeEvent(eventExpr = {
  # Monitorar TODOS os inputs que começam com "validar_ml_"
  # Pegamos o último input acionado
  session$lastInputId
}, {
  
  input_id <- session$lastInputId
  
  # Verificar se é um botão de validação
  if (!is.null(input_id) && grepl("^validar_ml_", input_id)) {
    
    # Extrair ID e tipo do botão
    # Formato: validar_ml_[NOTA_ID]_[TIPO]
    partes <- strsplit(input_id, "_")[[1]]
    
    if (length(partes) >= 4) {
      nota_id <- partes[3]  # Pega o terceiro elemento (NOTA_ID)
      tipo_str <- partes[4]  # Pega o quarto elemento (TIPO)
      tipo <- as.integer(tipo_str)
      
      cat("📝 Botão clicado:", input_id, "\n")
      cat("   ID extraído:", nota_id, "\n")
      cat("   Tipo extraído:", tipo, "\n")
      
      # Passar os dados atuais como parâmetro
      sucesso <- salvar_validacao_ml(
        registro_id = nota_id,
        tipo_validado = tipo,
        feedback = "boa",
        dados_resultados = values$resultados_lote  # ← Passar dados dinâmicos
      )
      
      if(sucesso) {
        showNotification(
          paste("✅ Validação salva:", nota_id, "→ Tipo", tipo),
          type = "message",
          duration = 3
        )
      } else {
        showNotification(
          paste("❌ Erro ao salvar validação para", nota_id),
          type = "error",
          duration = 5
        )
      }
...
observeEvent(input$confirmar_reset_ml, {
  ...
  observeEvent(input$classificar_lote, {
    ...
    tryCatch({
        ...
        for (i in 1:total) {
          ...
          classificacao <- tryCatch({
            res <- switch(
              ...
              "HIBRIDO"    = classificar_hibrido_com_modelo(texto, CONFIG_USUARIO()),  # Roteador inteligente
              "HIBRIDO_1"  = classificar_hibrido(texto, CONFIG_USUARIO()),            # Dicionário + API
              "HIBRIDO_2"  = classificar_hibrido(texto, CONFIG_USUARIO()),            # Dicionário + API
      ...
      conformes   <- as.numeric(stats["CONFORME"] %||% 0)
      divergentes <- as.numeric(stats["DIVERGENTE"] %||% 0)
      sem_ref     <- as.numeric(stats["SEM_REFERENCIA"] %||% 0)
      erros <- as.numeric(ifelse(is.na(stats["ERRO"]), 0, stats["ERRO"])) +
        as.numeric(ifelse(is.na(stats["<NA>"]), 0, stats["<NA>"]))
      cat("=== RESUMO FINAL ===\n")
      ...
      if (erros > 0) msg <- paste0(msg, " | Erros: ", erros)
      showNotification(msg, type = "message", duration = 12)
    }, error = function(e) {
