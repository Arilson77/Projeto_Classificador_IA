# ============================================================================
# Garantir carregamento do shiny para funções reativas
if (!"shiny" %in% .packages()) {
  suppressMessages(library(shiny))
}
# MARCAR TODAS E VALIDAR TODAS - LOTE DE VALIDAÇÃO ML IMPORTADO
# ============================================================================

# Marcar todas: seleciona todos os registros importados para visualização/validação
observeEvent(input$marcar_todas_validacao_ml, {
  req(values$importados_validacao_ml)
  # Considerar apenas última ocorrência de cada NOTA
  dados_lote <- values$importados_validacao_ml
  dados_lote <- dados_lote[!duplicated(dados_lote$nota, fromLast = TRUE), ]
  # Garantir que a coluna tipo_novo existe
  if (!'tipo_novo' %in% colnames(dados_lote)) {
    dados_lote$tipo_novo <- NA_integer_
  }
  # Atualizar cards de validação ML para mostrar apenas os importados
  output$cards_validacao_ml <- renderUI({
    if(nrow(dados_lote) == 0) {
      return(div("Nenhum registro importado para marcar."))
    }
    # Gerar cards para cada registro importado
...
# Validar todas: salva todas as validações importadas de uma vez
observeEvent(input$validar_todas_validacao_ml, {
  ...
  # Limpar visualização dos cards após validação
  output$cards_validacao_ml <- renderUI({ NULL })
  values$importados_validacao_ml <- NULL
})
# ============================================================================
# IMPORTAÇÃO DE ARQUIVO DE VALIDAÇÃO ML (NOTA, TIPO NOVO)
# ============================================================================
...
observeEvent(input$arquivo_validacao_ml, {
  ...
  tryCatch({
    ...
    # Armazenar para uso nos botões de marcação/validação em lote
...
# Função stub para evitar erro de função não encontrada
log_dual <- function(...) {
  # Placeholder: implementar lógica real se necessário
  return(NULL)
}
      ...
      if (total_validacoes %% 5 == 0) {
        ...
        tryCatch({
          ...
          if (resultado$sucesso) {
            cat(sprintf("✅ Modelo atualizado incrementalmente. Acurácia: %s%%\n", 
                        formatC(resultado$acuracia, format = "f", digits = 2, decimal.mark = ".")))
          }
        }, error = function(e) {
...
treinar_modelo_ml_incremental <- function() {
  ...
  tryCatch({
    ...
    cat(sprintf("   Acurácia: %s%% (baseada em %d validações recentes)\n", 
          formatC(acuracia, format = "f", digits = 2, decimal.mark = "."), nrow(dados_treino)))
    
    return(list(
      sucesso = TRUE,
      acuracia = as.numeric(formatC(acuracia, format = "f", digits = 2, decimal.mark = ".")),
...
treinar_modelo_ml <- function() {
  ...
  tryCatch({
    ...
    cat(sprintf("   Acurácia: %s%%\n", formatC(acuracia, format = "f", digits = 2, decimal.mark = ".")))
...
classificar_por_dicionario <- function(texto, dicionarios = DICIONARIOS_SAP)  {
  # Cache Dicionário
  cache_res <- cache_get(texto, tipo = "dicionario")
  if (!is.null(cache_res)) return(cache_res)
  ...
  cache_set(texto, resultado_dic, tipo = "dicionario")
...
extrair_assunto_principal <- function(texto)  {
  # Cache Assunto
  cache_res <- cache_get(texto, tipo = "assunto")
  if (!is.null(cache_res)) return(cache_res)
  ...
  if(isTRUE(config$usar_modelo_treinado)) {
    if(!is.null(validacoes_modelo$modelo_ativo)) {
      ...
      resultados$ml <- tryCatch({
        # Sempre prioriza ML, depois híbrido, depois dicionário
        classificar_hibrido_com_modelo(texto, config)
      }, error = function(e) {
...
classificar_hibrido_com_modelo <- function(texto, config) {
  # 1. Prioridade máxima: ML
  usar_ml <- config$usar_modelo_treinado && !is.null(validacoes_modelo$modelo_ativo) &&
    isTRUE(validacoes_modelo$configuracoes$usar_em_classificacao)
  if (usar_ml) {
    resultado_ml <- classificar_com_modelo_treinado(texto)
    if (!is.null(resultado_ml$tipo) && resultado_ml$metodo != "DICIONARIO_FALLBACK") {
      resultado_ml$metodo <- "MODELO_TREINADO"
      return(resultado_ml)
    }
    # Se ML falhar, continua para híbrido
    cat("⚠️ ML não retornou resultado, tentando híbrido...\n")
  }
  # 2. Híbrido (Dicionário + API)
  if (config$usar_dicionario && config$usar_api) {
    resultado_hibrido <- classificar_hibrido(texto, config)
    if (!is.null(resultado_hibrido$tipo)) {
      resultado_hibrido$metodo <- "HIBRIDO"
      return(resultado_hibrido)
    }
    cat("⚠️ Híbrido não retornou resultado, tentando dicionário...\n")
  }
  # 3. Dicionário puro
  if (config$usar_dicionario) {
    resultado_dic <- classificar_por_dicionario(texto, config$dicionarios)
    resultado_dic$metodo <- "DICIONARIO"
    return(resultado_dic)
  }
  # 4. Fallback
  cat("⚠️ Nenhuma configuração válida, usando dicionário como fallback\n")
          ...
          column(
            ...
            box(
              ...
              div(
                ...
                # Controles de filtro
                div(
                  style = "display: grid; grid-template-columns: 1fr 1fr auto auto; gap: 15px; margin-bottom: 25px;",
                  ...
                  div(
                    style = "padding-top: 25px;",
                    fileInput(
                      "arquivo_validacao_ml",
                      label = NULL,
                      accept = c(".csv", ".xlsx", ".xls"),
                      buttonLabel = "Importar Validação",
                      placeholder = "Arquivo de validação (NOTA, TIPO NOVO)"
                    )
                  )
                ),
                
                # Área de cards
                # Botões de marcação e validação em lote
                div(
                  style = "display: flex; gap: 10px; margin-bottom: 10px;",
                  actionButton(
                    "marcar_todas_validacao_ml",
                    label = div(icon("check-square"), "Marcar Todas"),
                    class = "btn-success",
                    style = "font-weight: 700;"
                  ),
                  actionButton(
                    "validar_todas_validacao_ml",
                    label = div(icon("check-double"), "Validar Todas"),
                    class = "btn-primary",
                    style = "font-weight: 700;"
                  )
                ),
...
server <- function(input, output, session)  {
  # Removido carregamento automático do modelo para lazy loading
  # ==========================================================================
  ...
  # Removido carregamento automático do modelo para lazy loading
  # Função para garantir que o modelo está carregado antes de classificar
  garantir_modelo_carregado <- function() {
    if (is.null(validacoes_modelo$modelo_ativo)) {
      carregou <- carregar_dados_modelo()
      if (carregou) {
        showNotification("✅ Modelo ML carregado do disco", type = "message", duration = 3)
      } else {
        showNotification("⚠️ Não foi possível carregar o modelo ML do disco", type = "error", duration = 5)
      }
    }
  }
  # garantir_modelo_carregado()
  # Exemplo: na função classificar_via_modelo_ml ou equivalente
  ...
  observeEvent(input$carregar_validacao_ml, {
    ...
    # Garantir que a coluna tipo_novo existe
    if (!'tipo_novo' %in% colnames(dados_validacao)) {
      dados_validacao$tipo_novo <- NA_character_
    }
    # Garantir que a coluna tipo_sugerido existe
    if (!'tipo_sugerido' %in% colnames(dados_validacao)) {
      dados_validacao$tipo_sugerido <- NA_character_
    }
    ...
    output$cards_validacao_ml <- renderUI({
      ...
      cards <- lapply(1:nrow(dados_validacao), function(i)  {
        ...
        # Defensive: garantir tipo_novo sempre existe
        tipo_novo <- if ("tipo_novo" %in% names(registro)) registro$tipo_novo else NA
        # Robust NA/NULL guards for all fields
        nota_key <- if (!is.null(registro$nota_key) && !is.na(registro$nota_key)) registro$nota_key else "-"
        # Sempre mostrar o tipo sugerido, mesmo se rejeitado
        tipo_sugerido <- if (!is.null(registro$tipo_sugerido) && !is.na(registro$tipo_sugerido) && registro$tipo_sugerido != "") registro$tipo_sugerido else if (!is.null(tipo_novo) && !is.na(tipo_novo) && tipo_novo != "") tipo_novo else "-"
        tipo_novo_valor <- tipo_sugerido
        confianca <- if (!is.null(registro$confianca) && !is.na(registro$confianca)) registro$confianca else "-"
        criticidade <- if (!is.null(registro$criticidade) && !is.na(registro$criticidade)) registro$criticidade else "-"
        categoria <- if (!is.null(registro$categoria) && !is.na(registro$categoria)) registro$categoria else "-"
        texto_completo <- if (!is.null(registro$texto_completo) && !is.na(registro$texto_completo)) registro$texto_completo else ""
        assunto_principal <- if (!is.null(registro$assunto_principal) && !is.na(registro$assunto_principal)) registro$assunto_principal else ""
        # Exibir sempre o motivo de rejeição detalhado, se houver
        motivo_rejeicao <- NULL
        if (!is.null(registro$motivo_rejeicao) && !is.na(registro$motivo_rejeicao) && nchar(registro$motivo_rejeicao) > 0) {
          motivo_rejeicao <- registro$motivo_rejeicao
        } else if (!is.null(registro$descricao) && !is.na(registro$descricao) && nchar(registro$descricao) > 0) {
          motivo_rejeicao <- registro$descricao
        }
            ...
            div(
              ...
              div(
                ...
                p(style = "margin: 0; opacity: 0.9; font-size: 13px;",
                  "Tipo IA sugerido: ", tipo_sugerido, " | ",
                  "Confiança: ", confianca, "% | ",
                  "Criticidade: ", criticidade),
                div(style = "margin-top: 4px; font-size: 13px; font-weight: 700; color: #1565C0;",
                    icon("tag"), " Tipo Novo: ", tipo_novo_valor),
                if (!is.null(motivo_rejeicao)) {
                  div(style = "margin-top: 4px; color: #c00; font-size: 12px; font-weight: 700;",
                      icon("exclamation-triangle"), " Motivo rejeição: ", motivo_rejeicao)
                }
              ),
          ...
          div(
            ...
            div(
              ...
              p(style = "margin: 0; font-size: 13px; color: #666; line-height: 1.6;",
                substr(texto_completo, 1, 200), 
                ifelse(nchar(texto_completo) > 200, "...", ""))
            ),
            
            # Assunto principal (se disponível)
            if (!is.null(assunto_principal) && !is.na(assunto_principal) && nchar(assunto_principal) > 0) {
              div(
                style = "margin-bottom: 20px;",
            ...
            div(
              ...
              div(
                ...
                lapply(1:6, function(tipo)  {
                  ...
                  estilo_extra <- if (!is.null(tipo_novo) && !is.na(tipo_novo) && tipo == tipo_novo) {
                  ...
                  actionButton(
                    paste0("validar_ml_", nota_key, "_", tipo),
                    label = div(
      ...
      withProgress(message = 'Classificando em lote...', value = 0, {
        for (i in 1:total) {
          ...
          classificacao <- tryCatch({
            ...
            # Validação de hífen/campo de seleção removida conforme solicitado
          ...
          # Lógica para exibir tipo sugerido mesmo se rejeitado, quando não houver campo_selecao
          # Sempre sugerir classificação, mesmo se rejeitado por hífen (exceto texto vazio)
          if (is.na(resultados$assunto_principal[i]) || resultados$assunto_principal[i] == "Sem texto") {
            resultados$tipo_novo[i] <- NA_integer_
            resultados$motivo_rejeicao[i] <- "Sem texto para classificar"
          } else {
            # Prioridade: ML > IA > Dicionário
            tipo_ml <- if (!is.null(classificacao$tipo_ml)) classificacao$tipo_ml else NA
            tipo_ia <- if (!is.null(classificacao$tipo_ia)) classificacao$tipo_ia else NA
            tipo_dic <- if (!is.null(classificacao$tipo_dic)) classificacao$tipo_dic else NA
            tipo_sugerido <- if (!is.na(tipo_ml)) tipo_ml else if (!is.na(tipo_ia)) tipo_ia else if (!is.na(tipo_dic)) tipo_dic else if (!is.null(classificacao$tipo_sugerido)) classificacao$tipo_sugerido else if (!is.null(classificacao$tipo_original)) classificacao$tipo_original else classificacao$tipo
            resultados$tipo_novo[i] <- tipo_sugerido
            # Validação do hífen APÓS a classificação (corrigida)
            if ("campo de seleção" %in% names(resultados) && !is.null(resultados[["campo de seleção"]][i]) && !is.na(resultados[["campo de seleção"]][i])) {
              campo_selecao_valor <- resultados[["campo de seleção"]][i]
              tem_hifen <- nchar(trimws(campo_selecao_valor)) > 0 && grepl("-", campo_selecao_valor, fixed = TRUE)
              if (!tem_hifen && !is.na(tipo_sugerido) && tipo_sugerido %in% c(5,6)) {
                resultados$tipo_novo[i] <- NA_integer_
                resultados$motivo_rejeicao[i] <- paste0("Tipo ", tipo_sugerido, " não permitido: campo de seleção sem hífen")
              } else {
                resultados$motivo_rejeicao[i] <- NA_character_
              }
            } else if (is.na(classificacao$tipo) && !is.null(classificacao$descricao)) {
              resultados$motivo_rejeicao[i] <- classificacao$descricao
            } else {
              resultados$motivo_rejeicao[i] <- NA_character_
            }
          }
  ...
  # Pré-carregar painel com último resultado salvo em memória (se existir)
  observe({
    if (is.null(values$resultados_lote)) {
      # Tentar carregar último resultado salvo em disco, se desejar persistência além da sessão
      # Exemplo: load('dados_modelo_treinado/ultimo_resultado_lote.RData')
      # if (exists('ultimo_resultado_lote')) values$resultados_lote <- ultimo_resultado_lote
      # Por padrão, manter apenas em memória
      return()
    }
    # Atualiza painel automaticamente sempre que houver dados em memória
  ...
  output$download_historico_excel <- downloadHandler(
    ...
    content = function(file)  {
      ...
      openxlsx::addWorksheet(wb, "Histórico")
      openxlsx::writeData(wb, "Histórico", dados)
