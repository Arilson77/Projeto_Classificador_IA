...
# (Conteúdo anterior do arquivo substituído pelo código consolidado e aprimorado enviado pelo usuário)
# Todas as melhorias de histórico, validação, métricas, outputs reativos, value boxes, gráficos, upload, cruzamento, processamento em lote e individual foram aplicadas conforme o código enviado.
# ================= DICIONÁRIO PETROBRAS REALISTA (TIPOS 1-6) ======================
DICIONARIO_PETROBRAS_REAL <- list(
  tipo_1 = list(
    descricao = "Serviço, apoio, rotina, limpeza, conservação, proteção",
    palavras_chave = c(
      "serviço", "apoio", "rotina", "limpeza", "preservação", "conservação", "proteção anticorrosiva"
    ),
    sinônimos = list(
      limpeza = c("dedetização", "sanitização", "descontaminação"),
      calibração = c("aferição", "ajuste de instrumento", "certificação"),
      lubrificação = c("engraxar", "aplicar óleo", "lubrificar")
    ),
    contexto = "Nota ZS, TI=1, sem impacto direto no processo"
  ),
  tipo_2 = list(
    descricao = "Melhorias, modificações, ajustes, upgrades, gestão de mudança",
    palavras_chave = c(
      "melhoria", "melhorar", "otimização", "otimizar",
      "upgrade", "modernização", "modernizar",
      "modificação", "modificar", "alteração", "alterar",
      "adequação", "adequar", "ajuste", "ajustar",
      "gestão de mudança", "GM", "projeto de alteração",
      "aperfeiçoamento", "incremento", "evolução"
    ),
    sinônimos = list(
      melhoria = c("otimização", "aperfeiçoamento", "evolução"),
      upgrade = c("modernização", "atualização", "incremento")
    ),
    contexto = "Nota ZS ou ZF, TI=2, exige GM quando significativa"
  ),
  tipo_3 = list(
    descricao = "Manutenção preventiva periódica, inspeção programada",
    palavras_chave = c(
      "preventiva", "manutenção preventiva", "plano de manutenção",
      "programada", "periódica", "rotina programada",
      "inspeção", "inspeção periódica", "inspeção preventiva",
      "verificação programada", "monitoramento periódico",
      "monitoramento de condição", "preditiva", "análise de óleo",
      "termografia", "vibração", "ultrassom",
      "semestral", "anual", "mensal", "quinzenal",
      "campanha", "parada programada"
    ),
    sinônimos = list(
      preventiva = c("programada", "planejada", "periódica"),
      inspeção = c("verificação", "vistoria", "checagem")
    ),
    contexto = "Nota ZS, gerada por plano SAP, TI=3"
  ),
  tipo_4 = list(
    descricao = "Intervenção de oportunidade, não programada mas planejada",
    palavras_chave = c(
      "oportunidade", "encaixe", "aproveitamento",
      "não programada", "fora da programação",
      "quebra de programação", "janela de oportunidade",
      "parada não prevista", "intervenção imediata planejada"
    ),
    sinônimos = list(
      oportunidade = c("encaixe", "aproveitamento de parada")
    ),
    contexto = "Nota ZF, TI=4, executada fora do plano mas com planejamento"
  ),
  tipo_5 = list(
    descricao = "Defeito: perda de eficiência SEM perda total de função",
    palavras_chave = c(
      "defeito", "defeituoso", "avaria", "anomalia",
      "falha incipiente", "falha degradada",
      "perda de eficiência", "desempenho reduzido",
      "degradação", "degradado", "operação degradada",
      "vazamento", "vazar", "gotejamento", "pingar",
      "ruído anormal", "vibração excessiva",
      "aquecimento", "sobreaquecimento moderado",
      "passagem interna", "desgaste", "corrosão",
      "não impossibilita", "perda parcial", "funcionamento comprometido",
      "com restrição", "operando com limitação"
    ),
    sinônimos = list(
      defeito = c("avaria", "anomalia", "falha degradada"),
      vazamento = c("gotejamento", "perda", "fuga")
    ),
    contexto = "Nota ZF, TI=5, equipamento AINDA opera"
  ),
  tipo_6 = list(
    descricao = "Falha: perda TOTAL de função, equipamento parado",
    palavras_chave = c(
      "falha", "falhou", "quebrou", "quebrado", "parado",
      "inoperante", "indisponível", "fora de operação",
      "perda total de função", "término da capacidade",
      "impossibilita", "não funciona", "não opera",
      "parada não programada", "parada de emergência",
      "impacto no processo", "perda de produção",
      "interferência na produção", "parada de unidade",
      "travamento", "travado", "ruptura", "rompido",
      "curto-circuito", "queimado", "explodido",
      "trinca", "fratura", "colapso"
    ),
    sinônimos = list(
      falha = c("quebra", "colapso", "ruptura"),
      parado = c("inoperante", "indisponível", "fora de serviço")
    ),
    contexto = "Nota ZF, TI=6, equipamento NÃO opera"
  ),
  contexto_geral = list(
    tipos_nota = c("ZS", "ZF", "ZO", "ZI", "ZR"),
    prioridades = c("A", "B", "C", "D", "ELEVADA", "MODERADA", "BAIXA", "MUITO BAIXA"),
    status_sistema = c("INIT", "MSPN", "MSPR", "CONF", "ENTE", "ENCE"),
    status_usuario = c("TRIA", "APRE", "SUSP", "CRIT", "IMED", "PRIA", "EXCL", "MREL"),
    disciplinas = c("Mecânica", "Elétrica", "Instrumentação", "Automação", "Civil")
  )
)

# ================= FUNÇÃO DE CLASSIFICAÇÃO COM EXPLICAÇÃO ======================
classificar_texto_real <- function(texto) {
  texto_lower <- tolower(texto)
  scores <- sapply(1:6, function(tipo) {
    palavras <- DICIONARIO_PETROBRAS_REAL[[paste0("tipo_", tipo)]]$palavras_chave
    sum(sapply(palavras, function(p) grepl(p, texto_lower, fixed = TRUE)))
  })
  tipo_sugerido <- which.max(scores)
  confianca <- if (sum(scores) > 0) round(100 * scores[tipo_sugerido] / sum(scores), 1) else 0
  palavras_match <- DICIONARIO_PETROBRAS_REAL[[paste0("tipo_", tipo_sugerido)]]$palavras_chave
  matches <- palavras_match[sapply(palavras_match, function(p) grepl(p, texto_lower, fixed = TRUE))]
  list(
    tipo = tipo_sugerido,
    confianca = confianca,
    explicacao = paste("Termos identificados:", paste(matches, collapse = ", ")),
    base_normativa = DICIONARIO_PETROBRAS_REAL[[paste0("tipo_", tipo_sugerido)]]$contexto
  )
}

# ================= SISTEMA HÍBRIDO DE CLASSIFICAÇÃO (VOTAÇÃO PONDERADA) ======================
library(httr)
library(jsonlite)
# library(randomForest) # Descomente se usar modelo ML

classificar_por_dicionario <- function(texto, dicionario = DICIONARIO_PETROBRAS_REAL) {
  texto_lower <- tolower(texto)
  scores <- sapply(1:6, function(tipo) {
    palavras <- dicionario[[paste0("tipo_", tipo)]]$palavras_chave
    sum(sapply(palavras, function(p) grepl(p, texto_lower, fixed = TRUE)))
  })
  if (sum(scores) == 0) {
    return(list(tipo = NA, confianca = 0, metodo = "dicionario", explicacao = "Nenhuma palavra-chave encontrada"))
  }
  tipo <- which.max(scores)
  confianca <- round(100 * scores[tipo] / sum(scores), 1)
  palavras_match <- dicionario[[paste0("tipo_", tipo)]]$palavras_chave
  matches <- palavras_match[sapply(palavras_match, function(p) grepl(p, texto_lower, fixed = TRUE))]
  list(
    tipo = tipo,
    confianca = confianca,
    metodo = "dicionario",
    explicacao = paste("Termos:", paste(matches, collapse = ", ")),
    votos = scores
  )
}

classificar_por_api <- function(texto, api_key = Sys.getenv("OPENAI_API_KEY")) {
  prompt <- paste0(
    "Você é um especialista em classificação de manutenção SAP Petrobras. ",
    "Classifique o texto abaixo em um dos 6 tipos de intervenção:\n",
    "1 = Serviço/Apoio/Rotina\n",
    "2 = Melhoria/Modificação\n",
    "3 = Preventiva/Inspeção periódica\n",
    "4 = Oportunidade\n",
    "5 = Defeito (sem perda total de função)\n",
    "6 = Falha (perda total de função)\n\n",
    "Texto: '", texto, "'\n\n",
    "Responda APENAS com um JSON no formato: {\"tipo\": N, \"confianca\": X, \"explicacao\": \"...\"}"
  )
  tryCatch({
    response <- POST(
      ...
      body = toJSON(list(
        model = "gpt-4",
        messages = list(
          list(role = "system", content = "Você classifica intervenções SAP Petrobras."),
    ...
    resposta <- fromJSON(result$choices[[1]]$message$content)
    list(
      tipo = as.integer(resposta$tipo),
      confianca = as.numeric(resposta$confianca),
      metodo = "api",
      explicacao = resposta$explicacao
    )
  }, error = function(e) {
    list(tipo = NA, confianca = 0, metodo = "api", explicacao = paste("Erro:", e$message))
  })
}

# classificar_por_ml: função exemplo, adapte ao seu modelo ML
classificar_por_ml <- function(texto, modelo = NULL) {
  tryCatch({
    features <- data.frame(
      texto = texto,
      tem_limpeza = grepl("limpeza|limpar", tolower(texto)),
      tem_falha = grepl("falha|falhou|parado", tolower(texto)),
      tem_defeito = grepl("defeito|vazamento|degradado", tolower(texto)),
      tem_preventiva = grepl("preventiva|inspeção|programada", tolower(texto)),
      tem_melhoria = grepl("melhoria|upgrade|modernização", tolower(texto)),
      stringsAsFactors = FALSE
    )
    predicao <- if (!is.null(modelo)) predict(modelo, features, type = "prob") else rep(0, 6)
    tipo <- if (!is.null(modelo)) which.max(predicao) else NA
    confianca <- if (!is.null(modelo)) round(100 * max(predicao), 1) else 0
    list(
      tipo = tipo,
      confianca = confianca,
      metodo = "ml",
      explicacao = if (!is.null(modelo)) paste("Probabilidades:", paste(names(predicao), round(predicao, 2), sep = "=", collapse = ", ")) else "Modelo ML não disponível"
    )
  }, error = function(e) {
    list(tipo = NA, confianca = 0, metodo = "ml", explicacao = paste("Erro:", e$message))
  })
}

classificar_hibrido <- function(texto, 
                                 pesos = c(dicionario = 1.0, api = 1.5, ml = 1.2),
                                 usar_api = TRUE,
                                 usar_ml = TRUE) {
  resultados <- list()
  resultados$dicionario <- classificar_por_dicionario(texto)
  if (usar_api) resultados$api <- classificar_por_api(texto)
  if (usar_ml) resultados$ml <- classificar_por_ml(texto)
  votos <- numeric(6)
  detalhes_voto <- list()
  for (metodo in names(resultados)) {
    res <- resultados[[metodo]]
    if (!is.na(res$tipo)) {
      peso <- pesos[[metodo]]
      voto_ponderado <- peso * (res$confianca / 100)
      votos[res$tipo] <- votos[res$tipo] + voto_ponderado
      detalhes_voto[[metodo]] <- list(
        tipo = res$tipo,
        peso = peso,
  ...
  tipo_final <- which.max(votos)
  confianca_final <- if (sum(votos) > 0) round(100 * votos[tipo_final] / sum(votos), 1) else 0
  list(
    tipo_final = tipo_final,
    confianca = confianca_final,
    votos = votos,
    resultados_individuais = resultados,
    detalhes_voto = detalhes_voto,
    consenso = length(unique(sapply(resultados, function(r) r$tipo))) == 1,
    timestamp = Sys.time()
  )
}

# ================= LOG DE CLASSIFICAÇÕES ======================
LOG_CLASSIFICACOES <- data.frame(
  timestamp = as.POSIXct(character()),
  texto = character(),
  tipo_final = integer(),
  confianca = numeric(),
  metodo_dicionario = integer(),
  metodo_api = integer(),
  metodo_ml = integer(),
  consenso = logical(),
  validado_usuario = integer(),
  stringsAsFactors = FALSE
)
registrar_classificacao <- function(resultado, texto, validacao_usuario = NA) {
  novo_registro <- data.frame(
    timestamp = resultado$timestamp,
    texto = substr(texto, 1, 500),
    tipo_final = resultado$tipo_final,
    confianca = resultado$confianca,
    metodo_dicionario = resultado$resultados_individuais$dicionario$tipo,
    metodo_api = ifelse(is.null(resultado$resultados_individuais$api), NA, resultado$resultados_individuais$api$tipo),
    metodo_ml = ifelse(is.null(resultado$resultados_individuais$ml), NA, resultado$resultados_individuais$ml$tipo),
    consenso = resultado$consenso,
    validado_usuario = validacao_usuario,
    stringsAsFactors = FALSE
  )
  LOG_CLASSIFICACOES <<- rbind(LOG_CLASSIFICACOES, novo_registro)
  write.csv(LOG_CLASSIFICACOES, "log_classificacoes.csv", row.names = FALSE)
}

# ================= PAINEL SHINY DE CURADORIA DE DICIONÁRIO ======================
# (Para rodar, salve este trecho em um arquivo separado ou adapte ao seu app principal)
# library(shiny)
# library(DT)
# library(shinyWidgets)
# SUGESTOES_PALAVRAS <- data.frame(
#   palavra = character(),
#   tipo_sugerido = integer(),
#   freq_aparicao = integer(),
#   exemplos = character(),
#   status = character(),
#   usuario_sugestao = character(),
#   data_sugestao = as.POSIXct(character()),
#   stringsAsFactors = FALSE
# )
# ui <- fluidPage(
#   titlePanel("📋 Curadoria de Dicionário - Classificação SAP Petrobras"),
#   tabsetPanel(
#     tabPanel("📖 Dicionário Atual",
#       br(),
#       selectInput("tipo_visualizar", "Tipo de Intervenção:", choices = 1:6, selected = 1),
#       verbatimTextOutput("info_tipo"),
#       DTOutput("tabela_palavras")
#     ),
#     tabPanel("➕ Sugerir Palavra",
#       br(),
#       textInput("nova_palavra", "Palavra/Expressão:", ""),
#       selectInput("tipo_novo", "Tipo de Intervenção:", choices = 1:6),
#       textAreaInput("exemplos_novo", "Exemplos de uso (opcional):", rows = 3),
#       actionButton("btn_sugerir", "Enviar Sugestão", class = "btn-primary"),
#       br(), br(),
#       verbatimTextOutput("msg_sugestao")
#     ),
#     tabPanel("✅ Revisar Sugestões",
#       br(),
#       DTOutput("tabela_sugestoes"),
#       br(),
#       fluidRow(
#         column(4, actionButton("btn_aprovar", "✅ Aprovar Selecionadas", class = "btn-success")),
#         column(4, actionButton("btn_rejeitar", "❌ Rejeitar Selecionadas", class = "btn-danger")),
#         column(4, actionButton("btn_atualizar", "🔄 Atualizar", class = "btn-info"))
#       ),
#       br(),
#       verbatimTextOutput("msg_revisao")
#     ),
#     tabPanel("📊 Análise",
#       br(),
#       h4("Palavras mais usadas (últimos 30 dias)"),
#       DTOutput("tabela_performance"),
#       br(),
#       h4("Taxa de acerto por tipo"),
#       plotOutput("plot_acertos")
#     )
#   )
# )
# 
...
# ================= INTEGRAÇÃO: VALIDAÇÃO EM LOTE E IMPORTAÇÃO ML ======================
# (Trechos integrados das versões intermediárias V14/V12)
server <- function(input, output, session)  {
  observeEvent(input$marcar_todas_validacao_ml, {
    req(values$importados_validacao_ml)
    dados_lote <- values$importados_validacao_ml
    dados_lote <- dados_lote[!duplicated(dados_lote$nota, fromLast = TRUE), ]
    if (!'tipo_novo' %in% colnames(dados_lote)) {
      dados_lote$tipo_novo <- NA_integer_
    }
    output$cards_validacao_ml <- renderUI({
      if(nrow(dados_lote) == 0) {
        return(div("Nenhum registro importado para marcar."))
      }
      lapply(1:nrow(dados_lote), function(i) {
        registro <- dados_lote[i, ]
        div(
          style = "margin-bottom: 15px; background: #f8f9fa; border-radius: 10px; padding: 15px;",
          strong("Nota: ", registro$nota, " | Tipo Validado: ", registro$tipo_novo)
        )
      })
    })
    notas_preview <- paste(head(dados_lote$nota, 10), collapse = ", ")
    msg <- paste0(
      "✅ ", nrow(dados_lote), " registros marcados para validação.\n",
      "Exemplo de NOTA: ", notas_preview, if (nrow(dados_lote) > 10) ", ..." else ""
    )
    showNotification(msg, type = "message", duration = 8)
  })
  
  observeEvent(input$validar_todas_validacao_ml, {
    req(values$importados_validacao_ml)
    dados_lote <- values$importados_validacao_ml
    dados_lote <- dados_lote[!duplicated(dados_lote$nota, fromLast = TRUE), ]
    if (!'tipo_novo' %in% colnames(dados_lote)) {
    ...
    total <- nrow(dados_lote)
    sucesso <- 0
    falha <- 0
    erros <- list()
    for(i in 1:total) {
      nota <- dados_lote$nota[i]
      tipo <- dados_lote$tipo_novo[i]
      res <- tryCatch({
        salvar_validacao_ml(nota, tipo)
        TRUE
      }, error = function(e) {
        erros[[length(erros)+1]] <<- paste0("NOTA ", nota, ": ", e$message)
        FALSE
      })
      if (isTRUE(res)) {
        sucesso <- sucesso + 1
    ...
    if (falha > 0) {
      detalhes <- paste(head(unlist(erros), 5), collapse = "\n")
      if (falha > 5) detalhes <- paste0(detalhes, "\n... e mais ", falha-5, " falhas.")
      msg <- paste0(msg, "\nDetalhes dos erros:\n", detalhes)
    }
    showNotification(msg, type = if (falha == 0) "message" else "warning", duration = 12)
    output$cards_validacao_ml <- renderUI({ NULL })
    values$importados_validacao_ml <- NULL
  })
  
  observeEvent(input$arquivo_validacao_ml, {
    req(input$arquivo_validacao_ml)
    arquivo <- input$arquivo_validacao_ml$datapath
    ext <- tools::file_ext(input$arquivo_validacao_ml$name)
    ...
    tryCatch({
      if (tolower(ext) %in% c("csv")) {
        dados_importados <- read.csv(arquivo, encoding = "UTF-8", stringsAsFactors = FALSE)
      } else if (tolower(ext) %in% c("xlsx", "xls")) {
        dados_importados <- readxl::read_excel(arquivo)
      } else {
        showNotification("❌ Formato de arquivo não suportado. Use CSV ou Excel.", type = "error")
        return()
      }
      total_linhas <- nrow(dados_importados)
      nomes <- tolower(gsub("[ ._-]+", "", names(dados_importados)))
      idx_nota <- which(nomes %in% c("nota", "notakey", "idnota"))
      idx_tipo <- which(nomes %in% c("tiponovo", "tipo", "tipovalidado"))
      if (length(idx_nota) == 0 || length(idx_tipo) == 0) {
        showNotification("❌ Arquivo deve conter colunas 'NOTA' e 'TIPO NOVO'!", type = "error")
        return()
      }
      dados_validacao <- data.frame(
        nota = as.character(dados_importados[[idx_nota]]),
        tipo_novo = as.integer(dados_importados[[idx_tipo]]),
        stringsAsFactors = FALSE
      )
      removidos_na <- sum(is.na(dados_validacao$nota) | is.na(dados_validacao$tipo_novo))
      dados_validacao <- dados_validacao[!is.na(dados_validacao$nota) & !is.na(dados_validacao$tipo_novo), ]
      if (nrow(dados_validacao) == 0) {
        showNotification("❌ Nenhum registro válido encontrado no arquivo.", type = "error")
        return()
      }
      values$importados_validacao_ml <- dados_validacao
      feedback_msg <- paste0(
        "✅ Arquivo importado com sucesso!\n",
        "Total de linhas no arquivo: ", total_linhas, "\n",
        if (removidos_na > 0) paste0(removidos_na, " linhas removidas por dados ausentes.\n") else "",
        "Registros válidos para validação: ", nrow(dados_validacao), "."
      )
      showNotification(feedback_msg, type = "message", duration = 8)
    }, error = function(e) {
      showNotification(paste0("❌ Erro ao importar arquivo: ", e$message), type = "error", duration = 10)
    })
  })
...
DICIONARIOS_SAP <- list(
  ...
  tipo_2 = list(
    ...
    palavras_chave = c(
      ...
      # ================= DICIONÁRIO NORMATIVO PETROBRAS ======================
      # Estrutura baseada nos documentos PE-3AGN-01315, PE-3AGN-01258, PE-3AGN-01777, PE-2LEP-00090, etc.
      # Este dicionário pode ser expandido conforme necessidade.
...
# ============================================================================
      ...
      # Função utilitária para consulta e explicação
      explica_classificacao <- function(campo, valor) {
        if (!is.null(dicionario_classificacao[[campo]]) && !is.null(dicionario_classificacao[[campo]][[valor]])) {
          info <- dicionario_classificacao[[campo]][[valor]]
          cat(paste0("Campo: ", campo, "\n"))
          cat(paste0("Valor: ", valor, "\n"))
          cat(paste0("Descrição: ", info$descricao, "\n"))
          cat(paste0("Referência: ", info$referencia, "\n"))
          if (!is.null(info$exemplos)) cat(paste0("Exemplos: ", paste(info$exemplos, collapse=", "), "\n"))
        } else {
          cat("Valor ou campo não encontrado no dicionário.\n")
        }
      }
      # ================= FIM DICIONÁRIO NORMATIVO ============================
  ...
  tryCatch({
    ...
    if (is.null(dados_resultados)) {
      ...
      if (!is.null(values_env)) {
        ...
        dados_busca <- values
        valores_disponivel <- !is.null(values$resultados_lote)
      }
...
classificar_hibrido_completo <- function(texto, config) {
  ...
  if(length(tipos_votados) == 1) {
    ...
    log_dual(paste("   ⚖️ DIVERGÊNCIA: Escolhido Tipo", tipo_final, "via", metodo_principal))
  }
  ...
  # Garantir que categoria nunca seja INDEFINIDO
  if(is.null(resultado_final$categoria) || is.na(resultado_final$categoria) || resultado_final$categoria == "INDEFINIDO") {
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      fluidRow(
        column(
          width = 12,
          actionButton("carregar_databricks", "Carregar via Databricks", icon = icon("cloud-upload"), class = "btn-success btn-lg"),
          tags$div(style = "margin-top:8px; color:#888; font-size:13px;", "(Usa credenciais Databricks para buscar dados IW28. Após carregar, lembre de importar o arquivo YSPM_TEXTOS para textos longos.)")
        )
      )
    )
  ),
...
server <- function(input, output, session)  {
  ...
  # --- Botão para carregar via Databricks no server ---
  observeEvent(input$carregar_databricks, {
    con <- conectar_databricks()
    if (!is.null(con)) {
      dados <- carregar_dados_databricks(con, QUERY_IW28)
      values$dados_ordens <- dados
      sparklyr::spark_disconnect(con)
      showNotification("Dados carregados via Databricks! Lembre-se de importar o arquivo YSPM_TEXTOS para adicionar o campo texto longo.", type = "warning", duration = 8)
    } else {
      showNotification("Erro ao conectar ao Databricks.", type = "error")
    }
  })
  # ...existing code...
...
# Função global de log para console e UI
log_dual <- function(msg) {
  cat(msg, "\n")
  log_fun <- getOption("log_ui_func", NULL)
  if (is.function(log_fun)) {
